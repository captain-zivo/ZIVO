...
"""ZIVO Official Bot v21 for Soroush Plus official Bot Platform.
...
━━━━━━━━━━━━━━━━━━
نسخه: zivo-official21 + zivo60.96.52
...
class BotCore(BotCore):
    ...
    def _admin_send_parts(self,user_id:str,chat_id:str)->str:
        ...
        if scope in {"private","groups","both","all"}:
            ...
            for idx,item in enumerate(items,1):
                ...
                if content.get("type")!="text":
                    ...
                    dest=self.ADMIN_MEDIA_DIR/f"official21_{int(time.time())}_{idx}_{random.randint(1000,9999)}{ext}"
        ...
        if scope in {"official","all"}:
            ...
            if ids:
                threading.Thread(target=self._official_broadcast_worker17,args=(ids,items),daemon=True,name="zivo-official21-broadcast").start(); pieces.append(f"🤖 Official: شروع ارسال به {len(ids):,} کاربر")
        ...
        if sender and chat==sender:
            ...
            if stage in {"welcome_text","welcome_photo","speaker_learn","speaker_delete"}:
                ...
                if stage=="welcome_photo":
                    ...
                    path=Path("/opt/zivo60/welcome_media")/f"official21_{gid}_{int(time.time())}.jpg"
