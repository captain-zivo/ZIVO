...
"""ZIVO Official Bot v14 for Soroush Plus official Bot Platform.
...
━━━━━━━━━━━━━━━━━━
نسخه: zivo-official14 + zivo60.96.46
...
MAIN_MENU = {
    "inline_keyboard": [
        ...
        [{"text": "🛡 مرکز کنترل", "callback_data": "ctl:current"}, {"text": "💎 خرید اشتراک", "callback_data": "prem:groups"}],
        [{"text": "🌱 اشتراک من", "callback_data": "prem:subs"}, {"text": "✨ مقایسه پلن‌ها", "callback_data": "menu:capabilities"}],
        [{"text": "📊 گزارش و امنیت", "callback_data": "menu:premiumtools"}, {"text": "🎮 سرگرمی", "callback_data": "menu:fun"}],
        [{"text": "🐾 اقتصاد میو", "callback_data": "menu:economy"}, {"text": "🧰 ابزارها", "callback_data": "menu:tools"}],
...
REQUIRED_JOIN_MENU = {
    "inline_keyboard": [
        ...
        [{"text": "✅ بررسی عضویت", "callback_data": "gate:check"}],
    ]
}

PREMIUM_TOOLS_MENU = {
    "inline_keyboard": [
        [{"text": "📊 گزارش امروز · GOLD", "callback_data": "ctl:q:report_today"}, {"text": "📈 گزارش هفتگی · DIAMOND", "callback_data": "ctl:q:report_week"}],
        [{"text": "🩺 سلامت گروه · DIAMOND", "callback_data": "ctl:q:group_health"}, {"text": "🧾 Admin Audit · GOLD", "callback_data": "ctl:q:admin_audit"}],
        ...
        [{"text": "💾 بکاپ دستی · GOLD", "callback_data": "ctl:q:backup_now"}, {"text": "♻️ Auto Backup · DIAMOND", "callback_data": "ctl:q:auto_backup_on"}],
        [{"text": "📦 وضعیت بکاپ", "callback_data": "ctl:q:backup_status"}, {"text": "👁 Watch List · DIAMOND", "callback_data": "ctl:q:watch_status"}],
        [{"text": "🗣 Funny · SILVER", "callback_data": "ctl:q:speaker_funny"}, {"text": "🎭 Anime · SILVER", "callback_data": "ctl:q:speaker_anime"}],
        [{"text": "🧠 AI روشن · GOLD", "callback_data": "ctl:q:ai_on"}, {"text": "🧠 AI خاموش", "callback_data": "ctl:q:ai_off"}],
        [{"text": "🧹 فیلتر محتوا · SILVER", "callback_data": "ctl:q:content_filter_help"}, {"text": "⌨️ همه فرمان‌ها", "callback_data": "ctl:raw"}],
...
class Store:
    def __init__(self, path: Optional[Path] = None) -> None:
        # Official14 deliberately keeps UI/session state in memory. Persistent
...
class BotCore:
    ...
    def _membership_gate_text(self, *, failed: bool = False) -> str:
        if failed:
            return (
                "🔒 فعال‌سازی پنل ZIVO کامل نشده\n"
                "━━━━━━━━━━━━━━━━━━\n"
                "هنوز عضویتت در کانال‌های رسمی تأیید نشده.\n\n"
                "📢 ZIVOHELP — خبرها، نسخه‌ها و اطلاعیه‌های مهم\n"
                "📚 ZIVOCMD — راهنما، فرمان‌ها و آموزش قابلیت‌ها\n\n"
                "روی دکمه‌های عضویت بزن، چند ثانیه صبر کن و بعد «✅ بررسی عضویت» را انتخاب کن.\n\n"
                "⏱ اگر همین الان عضو شدی، طبیعی است ثبت عضویت چند ثانیه زمان ببرد."
            )
        return (
            "👋 خوش اومدی به ZIVO\n"
            "━━━━━━━━━━━━━━━━━━\n"
            "ZIVO مرکز هوشمند مدیریت گروه‌های سروش+ است؛ از اتصال امن گروه و قفل‌ها تا ضداسپم، "
            "گزارش، سرگرمی، اقتصاد و پلن‌های حرفه‌ای، همه از یک پنل رسمی کنترل می‌شوند.\n\n"
            "برای بازشدن پنل فقط یک‌بار عضویت در دو کانال رسمی را تأیید کن:\n"
            "📢 ZIVOHELP — آپدیت‌ها و اطلاعیه‌ها\n"
            "📚 ZIVOCMD — راهنما و دستورات\n\n"
            "بعد از عضویت، «✅ بررسی عضویت» را بزن. پس از تأیید، این مرحله برایت تکرار نمی‌شود."
        )

    def _send_membership_gate(self, chat_id: str, *, failed: bool = False) -> str:
        text = self._membership_gate_text(failed=failed)
        self._send(chat_id, text, reply_markup=REQUIRED_JOIN_MENU)
        return text

    def _check_membership_gate(self, user_id: str, chat_id: str) -> str:
        try:
            result = self._ipc("main", {"op": "membership_check", "requester_user_id": int(user_id)}, timeout=18.0)
        except Exception as exc:
            text = "🟠 بررسی عضویت موقتاً به هسته نرسید. چند ثانیه بعد دوباره امتحان کن."
            self._send(chat_id, text, reply_markup=REQUIRED_JOIN_MENU)
            return text
        if result.get("ok") and result.get("allowed"):
            ...
            try:
                ...
                log.warning("persistent official gate pass failed | user=%s | %s: %s", user_id, type(exc).__name__, exc)
            text = (
                "✅ عضویت تأیید شد\n"
                "━━━━━━━━━━━━━━━━━━\n"
                "پنل کامل ZIVO برات فعال شد. از این به بعد لازم نیست هر بار عضویت رو دوباره بررسی کنی.\n\n"
                "👇 از پنل شروع کن"
            )
        ...
        if missing:
            text += "\n\nعضویت تأییدنشده: " + "، ".join(str(x) for x in missing)
        elif unknown:
            text += "\n\nوضعیت هنوز از سروش قطعی نشده؛ چند ثانیه بعد دوباره بررسی کن."
        self._send(chat_id, text, reply_markup=REQUIRED_JOIN_MENU)
        return text
    ...
    def _send_premium_tools_menu(self, user_id: str, chat_id: str) -> str:
        ...
        if active is None:
            text = "📊 گزارش و امنیت\n━━━━━━━━━━━━━━━━━━\nاول یک گروه رو از «گروه‌های من» انتخاب کن."
            self._send(chat_id, text, reply_markup=self._groups_menu(user_id))
            return text
        try:
            pst = self._ipc("main", {"op": "premium", "action": "status", "requester_user_id": int(user_id), "group_id": int(active.get("group_id") or 0)}, timeout=5.0)
            sub = pst.get("subscription") or {}
            plan_label = str(pst.get("plan_label") or sub.get("plan") or "رایگان")
        except Exception:
            plan_label = "نامشخص"
        text = (
            "📊 مرکز قابلیت‌های گروه\n"
            "━━━━━━━━━━━━━━━━━━\n"
            f"🏷 گروه فعال: {active.get('title') or active.get('group_id')}\n"
            f"💎 پلن فعلی: {plan_label}\n\n"
            "از همین صفحه قابلیت‌های حرفه‌ای را مستقیم کنترل کن.\n"
            "اگر سطح پلن کافی نباشد، Core قبل از اجرا درخواست ارتقا می‌دهد؛ هیچ دکمه‌ای نمایشی نیست."
        )
        self._send(chat_id, text, reply_markup=PREMIUM_TOOLS_MENU)
        return text

    def _send_main_menu(self, chat_id: str) -> str:
        text = (
            "⚡ ZIVO | مرکز فرماندهی گروه\n"
            "━━━━━━━━━━━━━━━━━━\n"
            "مدیریت حرفه‌ای گروه‌های سروش+، بدون شلوغی و بدون نیاز به رفتن داخل پیوی اکانت‌های اجرایی.\n\n"
            "🛡 مدیریت و امنیت\n"
            "قفل‌ها، ضداسپم، اخطار، سکوت، بن، فیلتر و پاکسازی\n\n"
            "🤖 هوشمندسازی\n"
            "سخنگو، گزارش‌ها، Anti-Raid، Auto Moderation و ابزارهای Premium\n\n"
            "🎮 سرگرمی و اقتصاد\n"
            "Meow، Pet، House و بازی‌های ZIVO\n\n"
            "💎 پلن گروه\n"
            "FREE قدرتمند است؛ SILVER، GOLD و DIAMOND قابلیت‌های حرفه‌ای‌تر را برای همان گروه باز می‌کنند.\n\n"
            "🔗 برای شروع، گروهت را وصل کن یا از «گروه‌های من» یکی از گروه‌های قبلی را انتخاب کن.\n"
            "👇 همه چیز از همین پنل کنترل می‌شود."
        )
        self._send(chat_id, text, reply_markup=MAIN_MENU)
        return text
    ...
    def handle_callback(self, raw: Dict[str, Any]) -> Optional[str]:
        ...
        if not self._is_owner(callback.sender_id) and self.store.gate_pending(callback.sender_id):
            return self._send_membership_gate(callback.chat_id, failed=True)
    ...
    def _premium_success_page(self, user_id: str, chat_id: str, order: Dict[str, Any], order_id: int) -> str:
        ...
        if gid > 0 and self._premium_group_row(user_id, gid) is not None:
            self.store.set_control_state(user_id, active_group_id=gid, mode="")
        plan = str(order.get("plan") or "").upper() or "PREMIUM"
        ...
        text = (
            ...
            "━━━━━━━━━━━━━━━━━━\n"
            "قابلیت‌های این پلن همین الان روی گروه باز شدند.\n"
            "از دکمه‌های پایین می‌تونی مستقیم وارد کنترل گروه یا امکانات پلن بشی."
        )
        markup = {"inline_keyboard": [
            [{"text": "🎛 کنترل همین گروه", "callback_data": "ctl:current"}, {"text": "✨ امکانات پلن", "callback_data": "menu:capabilities"}],
            [{"text": "🌱 اشتراک من", "callback_data": "prem:subs"}, {"text": "📊 گزارش و امنیت", "callback_data": "menu:premiumtools"}],
            [{"text": "🏠 پنل اصلی", "callback_data": "menu:home"}],
        ]}
        self._send(chat_id, text, reply_markup=markup)
        return text
        ...
        if message.chat_id == message.sender_id and not self._is_owner(message.sender_id):
            ...
            if contact_before is None and gate_state is None:
                ...
                if persistent.get("membership_passed"):
                    ...
                    return self._send_membership_gate(message.chat_id, failed=True)
                else:
                    try:
...
class ZivoOfficialApp:
    ...
    def run(self) -> None:
        ...
        log.info("startup | app=%s version=%s transport=official-bot-api bridge=unix-socket-direct ux=premium-tiers-v14", APP_NAME, VERSION)
