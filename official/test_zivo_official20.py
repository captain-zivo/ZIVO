...
import tempfile
import zivo_official20 as mod
assert mod.VERSION=='zivo-official20'
assert str(mod.BASE_DIR)=='/opt/ZIVO_OFFICIAL_BOT20'
...
def fake(account,p,timeout=45.0):
    ...
    if op=='official_admin':
        ...
        if a=='official_users': return {'ok':True,'user_ids':[60001]}
        if a=='inventory_refresh': return {'ok':True,'requested_accounts':list(mod.IPC_ACCOUNT_KEYS)}
        if a=='audience': return {'ok':True,'official':{'enabled':1},'account_network':{'groups':3,'private_enabled':3},'accounts':[],'active_campaigns':0}
...
# The real subscription callback must dispatch the Receipt Ledger card page.
# Official18/19 only tested the unused _premium_payment helper and therefore
# missed that prem:c:* still called the old _premium_pay implementation.
core.handle_callback(cb('60001','prem:c:11'))
labels=[b['text'] for row in t.sent[-1][2]['inline_keyboard'] for b in row]
assert any('پرداخت کردم' in x for x in labels)
assert 'پنل گروه' in t.sent[-1][1] and 'گروه تست' in t.sent[-1][1]
# photo-only subscription receipt: text is rejected, photo accepted and owner notified
core.handle_callback(cb('60001','receipt:start:11'))
r=core.handle(msg('60001','not a photo')); assert 'فقط عکس' in r
r=core.handle(msg('60001',photo=True)); assert 'رسید دریافت شد' in r and orders[11]['status']=='receipt_submitted'; assert any(x[1].get('file_id')=='PHOTO123' for x in t.media)
...
core.handle_callback(cb(owner,'payadmin:approve:11')); assert orders[11]['status']=='activated'
core.handle_callback(cb(owner,'payadmin:reverse:11')); assert orders[11]['status']=='reversed'
# Soroush admin panel must enqueue both group and private scope on every account.
core.handle_callback(cb(owner,'admin:adscope:both'))
core.handle_callback(cb(owner,'admin:adacct:all'))
core.handle(msg(owner,'تبلیغ کل شبکه'))
core.handle_callback(cb(owner,'admin:adpreview'))
core.handle_callback(cb(owner,'admin:adsend17'))
assert len(admin_calls)==1
assert admin_calls[0]['scope']=='both'
assert admin_calls[0]['account_keys']==list(mod.IPC_ACCOUNT_KEYS)
...
print('ZIVO OFFICIAL20 SUBSCRIPTION CARD RECEIPT + NETWORK CAMPAIGN + ASYNC CLEANUP TEST: PASS')
