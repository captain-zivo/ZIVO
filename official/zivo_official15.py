...
━━━━━━━━━━━━━━━━━━
نسخه: zivo-official15 + zivo60.96.46
...
class Store:
    def __init__(self, path: Optional[Path] = None) -> None:
        # Official15 deliberately keeps UI/session state in memory. Persistent
...
class BotCore:
    ...
    def _membership_gate_text(self, *, failed: bool = False) -> str:
        if failed:
            return (
                "📣 آخرین یادآوری عضویت ZIVO\n"
                "━━━━━━━━━━━━━━━━━━\n"
                "اگه هنوز عضو نشدی، الان بهترین موقعه؛ این دو کانال برای خبر نسخه‌ها، آموزش و فرمان‌های جدید ساخته شدن.\n\n"
                "📢 ZIVOHELP — آپدیت‌ها و اطلاعیه‌های مهم\n"
                "📚 ZIVOCMD — آموزش و فرمان‌های آماده\n\n"
                "این دومین و آخرین یادآوریه؛ پنل بعد از این پیام برایت باز می‌شه."
            )
        return (
            "👋 خوش اومدی به ZIVO\n"
            "━━━━━━━━━━━━━━━━━━\n"
            "ZIVO پنل هوشمند مدیریت گروه‌های سروش+ه؛ از قفل و ضداسپم تا پاکسازی، گزارش، AI، Backup، Anti-Raid، سرگرمی و اقتصاد میو رو از یک جای مرتب کنترل می‌کنی.\n\n"
            "برای اینکه خبر نسخه‌ها و آموزش قابلیت‌های جدید رو از دست ندی، پیشنهاد می‌کنم اول عضو دو کانال رسمی بشی:\n"
            "📢 ZIVOHELP — خبرها و آپدیت‌ها\n"
            "📚 ZIVOCMD — راهنما و دستورات\n\n"
            "بعد روی «✅ عضو شدم · ادامه» بزن. این پیام فقط دو بار نمایش داده می‌شه و بعد پنل همیشه مستقیم باز می‌شه."
        )
    ...
    def _send_membership_gate(self, chat_id: str, *, failed: bool = False) -> str:
        ...
        self._send(chat_id, text, reply_markup=REQUIRED_JOIN_MENU if not failed else {
            "inline_keyboard": [
                ...
                [{"text": "🚀 ورود به پنل ZIVO", "callback_data": "gate:check"}],
            ]
        })
        return text
    ...
    def _send_premium_tools_menu(self, user_id: str, chat_id: str) -> str:
        # Compatibility route: Official15 merged the old premium center into the
