...
"""ZIVO Official Bot v10 for Soroush Plus official Bot Platform.

Current official transport + account-control contract used here:
- HTTPS base: https://api.splus.ir/bot<TOKEN>/METHOD_NAME
- auth probe: getMe
- inbound: getUpdates long polling with offset
- outbound: sendMessage with chat_id + text

Group mutations are executed by the existing ZIVO account sessions through a shared SQLite Remote Command Bus; the official bot is the user-facing control plane.
"""
...
from zivo_multi_account import (
    ...
    recent_remote_control_jobs as multi_recent_remote_control_jobs,
    set_account_enabled as multi_set_account_enabled,
)

APP_NAME = "ZIVO Official"
VERSION = "zivo-official10"

# User explicitly requested an embedded token for this build.
BOT_TOKEN = "69669557:_Traf8PaLT5rQmxiIKrhQHV7GoXklGjGwsA"

BASE_DIR = Path("/opt/ZIVO_OFFICIAL_BOT10")
DB_PATH = Path(os.getenv("ZIVO_OFFICIAL_STATE_DB", "/var/lib/zivo-official/zivo_official_control.db"))
LOG_PATH = BASE_DIR / "zivo_official10.log"
# Existing account-bot shared control plane. BOT6 only writes control jobs here;
# it never opens an SPlusthon session itself.
CONTROL_DB_PATH = Path(os.getenv("ZIVO_ACCOUNT_CONTROL_DB", "/opt/zivo60/zivo_multi_accounts.db"))
BRIDGE_SOURCE_ACCOUNT = "official_bot"
...
━━━━━━━━━━━━━━━━━━
نسخه: zivo-official10

اتصال گروه
1) لینک خصوصی splus.ir/joingroup/... یا لینک عمومی گروه را در پیوی بات بفرست.
2) بات اکانت‌های آنلاین را با تعداد گروه نشان می‌دهد.
3) خودت انتخاب کن main / acc2 / acc3 کدام عضو شود.
4) بعد از Join، از «گروه‌های من» گروه را انتخاب و وارد «کنترل گروه» شو.

کنترل کامل گروه
در حالت کنترل، فرمان‌های واقعی zivo60.96.41 به Account Core ارسال می‌شوند؛ مثل:
قفل لینک | باز لینک | لیست قفل
اسپم فعال | اسپم خاموش | تنظیمات اسپم
پاکسازی | پاکسازی 500
خوشامد فعال | خوشامد خاموش
سخنگو روشن | سخنگو خاموش
راهنما | لیست دستورات | قوانین و سایر فرمان‌های اصلی

فرمان‌های ریپلای‌محور از پنل رسمی
هدف 123456 | بن
هدف 123456 | سکوت
هدف 123456 | اخطار
پیام 987 | پین
...
اکانت‌ها - فقط مالک کل
اکانت‌ها
اکانت [key] فعال
اکانت [key] خاموش
کاربران بات
پیام کاربر [user_id] [متن]

پیوی خود Account Botها در نسخه 96.40 خاموش است؛ رابط کاربر فقط همین بات رسمی است.
"""
...
CAPABILITY_STATUS_TEXT = """ZIVO | وضعیت Official v7 + Account 96.40
━━━━━━━━━━━━━━━━━━
فعال در این Release:
- Bot API رسمی getUpdates/sendMessage/Callback
- انتخاب دستی اکانت قبل از Join
- Join توسط Sessionهای واقعی main/acc2/acc3
- پیوی اکانت‌بات‌ها خاموش
- Remote Command Bus مشترک SQLite
- اجرای Router اصلی zivo60 برای کنترل گروه از بات رسمی
- کنترل فرمان‌های Reply-based با user_id/message_id
- پنل شیشه‌ای انتخاب گروه و کنترل سریع
- ضداسپم سریع‌تر: Ban در مسیر realtime و پاکسازی در Worker پس‌زمینه
- پاکسازی سابقه اسپمر به‌صورت bounded history scan

خروجی فرمان‌های Account Core داخل خود گروه ارسال می‌شود؛ بات رسمی نتیجه اجرای Job را هم در پیوی اعلام می‌کند.
"""
...
class Store:
    ...
    def _connect(self) -> sqlite3.Connection:
        # 96.42/Official10: keep the official control state beside the shared
        # account control DB, not inside the replaceable release directory.
        # If the configured parent temporarily disappears during a cutover,
        # recreate it once before opening SQLite instead of entering a hot
        # notifier error loop.
        self.path.parent.mkdir(parents=True, exist_ok=True)
        try:
            con = sqlite3.connect(self.path, timeout=15)
        except sqlite3.OperationalError:
            fallback = Path("/var/lib/zivo-official/zivo_official_control.db")
            fallback.parent.mkdir(parents=True, exist_ok=True)
            if self.path != fallback:
                log.warning("official state DB fallback | configured=%s fallback=%s", self.path, fallback)
                self.path = fallback
            con = sqlite3.connect(self.path, timeout=15)
...
class BotCore:
    def __init__(self, store: Store, transport: Optional[SoroushOfficialTransport], control_db: Path = CONTROL_DB_PATH) -> None:
        self.store = store
        self.transport = transport
        self.control_db = Path(control_db)
        self._spam: Dict[tuple[str, str], Deque[float]] = defaultdict(deque)
        social_games.configure(Path(os.getenv("ZIVO_OFFICIAL_SOCIAL_DB", "/opt/zivo60/zivo_official_social.db")), global_owner_id=49145577, bot_user_ids={69669557})
    ...
    def _account_rows(self) -> list[sqlite3.Row]:
        try:
            if not self.control_db.exists():
                return []
            return list(multi_list_accounts(self.control_db))
        except Exception as exc:
            log.warning("control DB account list failed | %s", exc)
            return []

    def _account_status_text(self, *, detailed: bool = False) -> str:
        rows = self._account_rows()
        if not rows:
            return f"هیچ اکانت ZIVO در Control DB پیدا نشد.\nمسیر: {self.control_db}"
    ...
    def _managed_rows_for_user(self, user_id: str) -> list[sqlite3.Row]:
        if self._is_owner(user_id):
            # Deduplicate global-owner view by group id while preserving newest.
            seen: set[int] = set()
            out: list[sqlite3.Row] = []
            for row in self.store.all_managed_groups():
                ...
                out.append(row)
            return out
        return self.store.managed_groups(user_id)
    ...
    def _begin_group_link(self, message: IncomingMessage, kind: str, value: str) -> str:
        ...
        try:
            if not self.control_db.exists():
                text = f"Control DB اکانت‌ها پیدا نشد: {self.control_db}"
                self._send(message.chat_id, text)
                return text
            candidates = list(multi_eligible_join_accounts(self.control_db, exclude_account_key=BRIDGE_SOURCE_ACCOUNT))
        except Exception as exc:
            log.exception("eligible account lookup failed")
            text = f"خطا در خواندن وضعیت اکانت‌ها: {type(exc).__name__}"
            self._send(message.chat_id, text)
            return text
        if not candidates:
            text = "فعلاً هیچ اکانت ZIVO آنلاین و دارای ظرفیت Join نیست."
            self._send(message.chat_id, text)
            return text
        ...
        lines = [
            "لینک دریافت شد. کدام اکانت عضو گروه شود؟",
            f"کد انتخاب: {request_id}",
            "━━━━━━━━━━━━━━━━━━",
        ]
        for row in candidates[:12]:
            lines.append(
                f"• {row['account_key']} | {row['label'] or '-'} | گروه‌ها: {int(row['groups_count'] or 0)}"
            )
        text = "\n".join(lines)
        self._send(message.chat_id, text, reply_markup=self._account_selection_markup(request_id, candidates))
        log.info("official bridge awaiting account choice | request=%s user=%s kind=%s", request_id, message.sender_id, kind)
        return text
    ...
    def _select_join_account(self, callback: IncomingCallback, request_id: int, account_key: str) -> str:
        ...
        if pending is None or str(pending["status"] or "") != "waiting_account":
            text = "این درخواست انتخاب اکانت منقضی یا قبلاً استفاده شده است."
            self._send(callback.chat_id, text, reply_markup=MAIN_MENU)
            return text
        key = str(account_key or "").strip().lower()
        try:
            candidates = {
                str(row["account_key"] or "").strip().lower(): row
                for row in multi_eligible_join_accounts(self.control_db, exclude_account_key=BRIDGE_SOURCE_ACCOUNT)
            }
        except Exception as exc:
            text = f"خواندن وضعیت اکانت‌ها ناموفق بود: {type(exc).__name__}"
            self._send(callback.chat_id, text)
            return text
        if key not in candidates:
            text = f"اکانت {key} الان آنلاین/دارای ظرفیت Join نیست. دوباره لینک را بفرست و اکانت دیگری انتخاب کن."
            self._send(callback.chat_id, text, reply_markup=MAIN_MENU)
            return text
        try:
            job_id = multi_create_join_job(
                self.control_db, source_account=BRIDGE_SOURCE_ACCOUNT, target_account=key,
                requester_user_id=int(callback.sender_id), source_message_id=int(callback.message_id or 0),
                link_kind=str(pending["link_kind"]), link_value=str(pending["link_value"]),
            )
            self.store.add_bridge_request(
                job_id=job_id, user_id=callback.sender_id, chat_id=callback.chat_id,
                target_account=key, link_kind=str(pending["link_kind"]), link_value=str(pending["link_value"]),
            )
            self.store.mark_pending_group_link_selected(int(request_id), key)
        except Exception as exc:
            log.exception("bridge join enqueue failed")
            text = f"ثبت درخواست Join ناموفق بود: {type(exc).__name__}"
            self._send(callback.chat_id, text)
            return text
        text = (
            "در حال اتصال اکانت به گروه...\n"
            f"اکانت انتخابی: {key}\n"
            f"کد Job: {job_id}\n"
            "این Join در مسیر فوری اجرا می‌شود و نتیجه واقعی همین‌جا می‌آید."
        )
        self._send(callback.chat_id, text, reply_markup=MAIN_MENU)
        log.info("official bridge join queued | job=%s user=%s account=%s", job_id, callback.sender_id, key)
        return text

    def _active_group(self, user_id: str) -> Optional[sqlite3.Row]:
        state = self.store.control_state(user_id)
        gid = int(state.get("active_group_id") or 0)
        if gid <= 0:
            return None
        return self.store.managed_group(user_id, gid, owner_override=self._is_owner(user_id))
    ...
    def _queue_remote_command(self, *, user_id: str, chat_id: str, command_text: str) -> str:
        ...
        try:
            job_id = multi_create_remote_control_job(
                self.control_db,
                target_account=str(row["account_key"]),
            ...
            self.store.add_remote_request(
                job_id=job_id, user_id=user_id, chat_id=chat_id,
                group_id=int(row["group_id"]), account_key=str(row["account_key"]),
                command_text=command_text,
            )
        except Exception as exc:
            log.exception("remote command enqueue failed")
            return f"ثبت فرمان در Control Bus ناموفق بود: {type(exc).__name__}"
        log.info(
            "official remote command queued | job=%s user=%s group=%s account=%s command=%r target_user=%s target_message=%s",
            job_id, user_id, int(row["group_id"]), row["account_key"], command[:120], target_user_id, target_message_id,
        )
        return f"فرمان در صف اجرای اکانت قرار گرفت.\nJob: {job_id}\nفرمان: {command[:600]}"

    def poll_bridge_jobs(self, sender: Optional[SoroushOfficialTransport] = None) -> int:
        tx = sender or self.transport
        if tx is None or not self.control_db.exists():
            return 0
        notified = 0
        for req in self.store.pending_bridge_requests():
            try:
                row = multi_get_join_job(self.control_db, int(req["job_id"]))
            except Exception as exc:
                log.warning("bridge job read failed | job=%s | %s", req["job_id"], exc)
                continue
            if row is None:
                continue
            status = str(row["status"] or "")
            if status not in {"done", "failed", "capacity"}:
                continue
            code = str(row["result_code"] or status)
            gid = int(row["result_group_id"] or 0)
            title = str(row["result_title"] or "").strip()
            count = int(row["result_member_count"] or -1)
            target_account = str(req["target_account"] or "")
            if status == "done":
                actual_account = target_account
                if gid > 0:
                    try:
                        claim = multi_lookup_group_claim(self.control_db, group_id=gid)
                    except Exception:
                        claim = None
                    if claim is not None and str(claim["account_key"] or "").strip():
                        actual_account = str(claim["account_key"] or "").strip().lower()
                    if code != "joined_pending":
                        self.store.add_managed_group(
                            user_id=str(req["user_id"]), group_id=gid, account_key=actual_account,
                            title=title, member_count=count,
                        )
                        self.store.set_control_state(str(req["user_id"]), active_group_id=gid)
                if code == "joined_full":
                    text = "اتصال گروه انجام شد و ZIVO با دسترسی FULL آماده است.\n"
                elif code == "joined_basic":
                    text = (
                        "اتصال گروه انجام شد و ZIVO در حالت BASIC آماده است.\n"
                        "برای بن/سکوت/پین/پاکسازی مدیریتی، اکانت اجرایی را داخل گروه ادمین کامل کن.\n"
                    )
                elif code == "joined_pending":
                    text = (
                        "اکانت عضو گروه شد، اما دسترسی نوشتن/فعال‌سازی هنوز تأیید نشده است.\n"
                        "اجازه ارسال پیام را بررسی کن و اکانت اجرایی را ادمین کامل کن.\n"
                    )
                elif code == "joined":
                    text = "اتصال گروه انجام شد.\n"
                elif code == "already":
                    text = "گروه از قبل روی یکی از اکانت‌های ZIVO فعال/شناخته‌شده بود و به پنل تو اضافه شد.\n"
                else:
                    text = f"درخواست Join پایان یافت. نتیجه: {code}\n"
                if title:
                    text += f"گروه: {title}\n"
                if gid > 0:
                    text += f"شناسه گروه: {gid}\n"
                text += f"اکانت اجرایی: {actual_account}\nکد درخواست: {int(req['job_id'])}"
                if gid > 0 and code != "joined_pending":
                    text += "\n\nگروه به عنوان گروه فعال پنل انتخاب شد و فرمان‌ها از همین حالا قابل ارسال‌اند."
            elif status == "capacity":
                text = f"اکانت {target_account} فعلاً به محدودیت Join خورده است. کد درخواست: {int(req['job_id'])}"
            else:
                err = str(row["last_error"] or "")[:300]
                text = f"اتصال گروه ناموفق بود.\nاکانت: {target_account}\nکد درخواست: {int(req['job_id'])}"
                if err:
                    text += f"\nخطا: {err}"
            try:
                markup = GROUP_CONTROL_MENU if status == "done" and gid > 0 else MAIN_MENU
                tx.send_text(str(req["chat_id"]), text, reply_markup=markup)
            except Exception as exc:
                log.warning("bridge notification send failed | job=%s | %s", req["job_id"], exc)
                continue
            self.store.mark_bridge_notified(int(req["job_id"]), status)
            notified += 1
            log.info("bridge request notified | job=%s status=%s code=%s", req["job_id"], status, code)
        return notified
    ...
    def poll_remote_jobs(self, sender: Optional[SoroushOfficialTransport] = None) -> int:
        ...
        if tx is None or not self.control_db.exists():
            return 0
        notified = 0
        for req in self.store.pending_remote_requests():
            try:
                row = multi_get_remote_control_job(self.control_db, int(req["job_id"]))
            except Exception as exc:
                log.warning("remote job read failed | job=%s | %s", req["job_id"], exc)
                continue
            if row is None:
                continue
            status = str(row["status"] or "")
            ...
            if status == "done":
                text = (
                    "اجرای فرمان توسط اکانت انجام شد.\n"
                    f"Job: {int(req['job_id'])}\n"
                    f"گروه: {int(req['group_id'])}\n"
                    f"اکانت: {req['account_key']}\n"
                    f"نتیجه: {str(row['result_text'] or 'OK')[:1200]}"
                )
            else:
                result = str(row["result_text"] or "اجرای فرمان ناموفق بود.")[:900]
                err = str(row["last_error"] or "")[:500]
                text = (
                    "فرمان اجرا نشد.\n"
                    f"Job: {int(req['job_id'])}\n"
                    f"کد: {code}\n"
                ...
                if err:
                    text += f"\nجزئیات: {err}"
            try:
                tx.send_text(str(req["chat_id"]), text, reply_markup=GROUP_CONTROL_MENU)
            except Exception as exc:
                log.warning("remote result notification failed | job=%s | %s", req["job_id"], exc)
                continue
            self.store.mark_remote_notified(int(req["job_id"]), status)
            notified += 1
            log.info("remote request notified | job=%s status=%s code=%s", req["job_id"], status, code)
        return notified
    ...
    def handle_callback(self, raw: Dict[str, Any]) -> Optional[str]:
        ...
        if data.startswith("ctl:group:"):
            try:
                ...
                return None
            row = self.store.managed_group(
                callback.sender_id, gid, owner_override=self._is_owner(callback.sender_id)
            )
            if row is None:
    ...
    def _route_command(self, message: IncomingMessage) -> Optional[str]:
        ...
        social = social_games.parse_social_command(text, user_id=int(message.sender_id))
        if social is not None:
            action = str(social.get("action") or "")
            uid = int(message.sender_id)
            if action == "meow_claim": return social_games.claim_meow(uid)
            if action == "meow_profile": return "ZIVO | دارایی من\n" + social_games.profile_section(uid)
            calls = {
                "pet_shop": (social_games.pet_shop_text, ()),
                "pet_buy": (social_games.buy_pet, (uid, str(social.get("spec") or ""))),
                "pet_status": (social_games.pet_status, (uid,)),
                "pet_name": (social_games.name_pet, (uid, str(social.get("name") or ""))),
                "pet_feed": (social_games.feed_pet, (uid,)),
                "pet_play": (social_games.play_with_pet, (uid,)),
                "pet_call": (social_games.pet_call, (uid, str(social.get("call") or "name"))),
                ...
                "house_buy": (social_games.buy_house, (uid, str(social.get("spec") or ""))),
                "house_sell_instant": (social_games.instant_sell_house, (uid, int(social.get("ownership_id") or 0))),
                "house_list": (social_games.list_house, (uid, int(social.get("ownership_id") or 0), int(social.get("price") or 0))),
                "house_cancel_listing": (social_games.cancel_house_listing, (uid, int(social.get("listing_id") or 0))),
                "house_buy_listing": (social_games.buy_listed_house, (uid, int(social.get("listing_id") or 0))),
            }
            if action in calls:
                fn,args = calls[action]; return fn(*args)
            if action in {"ttt_start","ttt_join","ttt_cancel","ttt_status","ttt_move","ship","wager_start","wager_accept","wager_cancel"}:
                return "این قابلیت گروهی آماده انتقال است، اما Bot فعلی هنوز اجازه ورود/خواندن کامل گروه را ندارد."
            if action in {"market","distance","transfer_prepare","transfer_confirm","transfer_cancel"}:
                return "این بخش در موج بعدی انتقال Official فعال می‌شود."
        ...
        if match:
            ...
            if key not in rows:
                return f"اکانت {key} در Control DB ثبت نشده است."
            multi_set_account_enabled(self.control_db, key, enabled)
            return f"مسیر Shared Control اکانت {key} {'فعال' if enabled else 'خاموش'} شد. سرویس اکانت با این فرمان Stop نمی‌شود."
        if low == "کاربران بات":
            if not self._is_owner(message.sender_id):
...
class ZivoOfficialApp:
    def __init__(self) -> None:
        ...
        self.notifier_transport = SoroushOfficialTransport()
        self.store = Store()
    ...
    def _bridge_notifier_loop(self) -> None:
        while not self.stop_event.is_set():
            try:
                self.core.poll_bridge_jobs(self.notifier_transport)
                self.core.poll_remote_jobs(self.notifier_transport)
            except sqlite3.OperationalError as exc:
                log.error("bridge/control notifier sqlite failure | state_db=%s control_db=%s error=%s", self.store.path, self.core.control_db, exc)
                self.stop_event.wait(1.0)
                continue
            except Exception:
                log.exception("bridge/control notifier loop failed")
            self.stop_event.wait(0.08)

    def run(self) -> None:
        signal.signal(signal.SIGTERM, self.request_stop)
        signal.signal(signal.SIGINT, self.request_stop)
        log.info("startup | app=%s version=%s transport=official-bot-api", APP_NAME, VERSION)
        log.info("runtime paths | pid=%s core=%s state_db=%s control_db=%s", os.getpid(), Path(__file__).resolve(), self.store.path, self.core.control_db)
        ...
        if self.core.control_db.exists():
            log.info("account control DB detected | path=%s accounts=%s", self.core.control_db, len(self.core._account_rows()))
        else:
            log.warning("account control DB not found | path=%s", self.core.control_db)
        self.notifier_thread = threading.Thread(target=self._bridge_notifier_loop, name="zivo-official-bridge-notifier", daemon=True)
        ...
        if self.notifier_thread is not None and self.notifier_thread.is_alive():
            self.notifier_thread.join(timeout=3.0)
        try:
            self.transport.session.close()
            self.notifier_transport.session.close()
        except Exception:
            pass
