...
"""ZIVO Official Bot v22 for Soroush Plus official Bot Platform.
...
━━━━━━━━━━━━━━━━━━
نسخه: zivo-official22 + zivo60.96.53
...
👛 کیف پول ریالی
• شارژ با زیبال به‌صورت خودکار یا کارت‌به‌کارت با رسید و بررسی مدیریت
• خرید اشتراک، Meow و کد هدیه Meow با موجودی کیف پول
• نمایش موجودی و گردش‌های ثبت‌شده داخل بات
...
کیف پول | شارژ کیف پول | موجودی کیف پول
...
━━━━━━━━━━━━━━━━━━
🆓 FREE — رایگان
• اولویت پاسخ پایه در صف شلوغ ربات
...
🥈 SILVER — نقره‌ای
• اولویت پاسخ نقره‌ای؛ جلوتر از FREE
• همه FREE
...
🥇 GOLD — طلایی
• اولویت پاسخ سریع؛ جلوتر از SILVER و FREE
• همه FREE + SILVER
...
💎 DIAMOND — الماس
• بالاترین اولویت پاسخ و مسیر سریع در شلوغی
• همه قابلیت‌های قبلی
...
🔐 این تفاوت‌ها فقط متن نیستند؛ Core 96.53 هم دسترسی قابلیت‌ها و هم اولویت صف پاسخ را براساس پلن واقعی گروه اجرا می‌کند.
"""
...
MAIN_MENU = {
    "inline_keyboard": [
        ...
        [{"text": "👛 کیف پول من", "callback_data": "wallet:home"}, {"text": "➕ شارژ کیف پول", "callback_data": "wallet:topup"}],
...
PLAN_UX = {
    "free": {
        ...
        "features": [
            ...
            "خوشامد متنی", "سخنگوی Normal", "سرگرمی و اقتصاد پایه", "اولویت پاسخ پایه",
        ],
    },
    "silver": {
        ...
        "features": [
            ...
            "Learning و Custom Q&A سخنگو", "Pet با 10٪ تخفیف و Meow Luck 1.15×", "اولویت پاسخ نقره‌ای",
        ],
    },
    "gold": {
        ...
        "features": [
            "همه قابلیت‌های SILVER", "پاکسازی تا 5000 پیام + پاکسازی کامل", "اولویت پاسخ سریع",
    ...
    "diamond": {
        ...
        "features": [
            ...
            "AI کامل و شخصیت اختصاصی", "Auto Moderation", "Watch List", "بالاترین اولویت پاسخ",
...
class BotCore:
    ...
    def _admin_send_ad(self, user_id: str, chat_id: str) -> str:
        ...
        if scope in {"official","all"}:
            try:
                ...
                ids=[]
            if ids:
                threading.Thread(target=self._official_broadcast_worker,args=(ids,body),daemon=True,name="zivo-official18-broadcast").start()
    ...
    def _order_status_fa(status: str) -> str:
        return {
            ...
            "cancelled": "لغوشده",
            "rejected": "ردشده",
            "awaiting_receipt": "منتظر ارسال رسید",
            "receipt_submitted": "رسید در حال بررسی",
            "reversed": "تأیید برگشت‌خورده",
...
class BotCore(BotCore):
    ...
    def _admin_send_parts(self,user_id:str,chat_id:str)->str:
        ...
        if scope in {"private","groups","both","all"}:
            ...
            for idx,item in enumerate(items,1):
                ...
                if content.get("type")!="text":
                    ...
                    dest=self.ADMIN_MEDIA_DIR/f"official22_{int(time.time())}_{idx}_{random.randint(1000,9999)}{ext}"
        ...
        if scope in {"official","all"}:
            ...
            if ids:
                threading.Thread(target=self._official_broadcast_worker17,args=(ids,items),daemon=True,name="zivo-official22-broadcast").start(); pieces.append(f"🤖 Official: شروع ارسال به {len(ids):,} کاربر")
...
# ---------------------------------------------------------------------------
# Official18: low-latency caches, photo-only manual receipts, reversible admin
# order ledger, Gold welcome designer and profanity-safe speaker learning.
# ---------------------------------------------------------------------------
class BotCore(BotCore):
    RECEIPT_REASONS = {
        "amount": "مبلغ واریزی با مبلغ سفارش مطابقت ندارد",
        "invalid": "رسید نامعتبر یا ناخوانا است",
        "notfound": "پرداخت در حساب مقصد پیدا نشد",
        "duplicate": "این رسید قبلاً برای سفارش دیگری استفاده شده",
        "mismatch": "اطلاعات رسید با سفارش مطابقت ندارد",
    }

    def __init__(self, store: Store, transport: Optional[SoroushOfficialTransport]) -> None:
        super().__init__(store, transport)
        self._fast_cache_lock = threading.RLock()
        self._fast_account_cache: tuple[float, list[Dict[str, Any]]] = (0.0, [])
        self._fast_groups_cache: Dict[str, tuple[float, list[Dict[str, Any]]]] = {}
        self._fast_plan_cache: Dict[tuple[str,int], tuple[float, Dict[str,Any]]] = {}
        self._receipt_state: Dict[str, Dict[str, Any]] = {}
        self._custom_state: Dict[str, Dict[str, Any]] = {}

    # ---- latency ----
    def _account_rows(self) -> list[Dict[str, Any]]:
        now=time.monotonic()
        with self._fast_cache_lock:
            ts, cached=self._fast_account_cache
            if cached and now-ts < 3.0:
                return [dict(x) for x in cached]
        from concurrent.futures import ThreadPoolExecutor, as_completed
        def probe(key: str) -> Optional[Dict[str, Any]]:
            started=time.monotonic()
            try: data=self._ipc(key,{"op":"status"},timeout=0.85)
            except Exception: return None
            if not data.get("ok"): return None
            return {"account_key":str(data.get("account_key") or key),"label":str(data.get("account_label") or key),"enabled":1 if data.get("enabled") else 0,
                    "self_id":int(data.get("self_id") or 0),"status":"online" if data.get("connected") else "offline","last_heartbeat_at":datetime.now(timezone.utc).isoformat(),
                    "groups_count":int(data.get("groups_count") or 0),"socket":str(ipc_socket_path(key)),"latency_ms":round((time.monotonic()-started)*1000.0,1)}
        rows=[]
        with ThreadPoolExecutor(max_workers=len(IPC_ACCOUNT_KEYS),thread_name_prefix="zivo-ipc-probe") as pool:
            futs=[pool.submit(probe,key) for key in IPC_ACCOUNT_KEYS]
        ...
        rows.sort(key=lambda r:(0 if int(r.get("enabled") or 0) and r.get("status")=="online" and int(r.get("self_id") or 0)>0 else 1,float(r.get("latency_ms") or 99999)))
        for idx,row in enumerate(rows):
            row["speed_label"]=self._speed_label(float(row.get("latency_ms") or 99999),idx); row["recommended"]=idx==0 and row.get("status")=="online" and int(row.get("enabled") or 0)==1
        with self._fast_cache_lock: self._fast_account_cache=(now,[dict(x) for x in rows])
        return rows

    def _managed_rows_for_user(self,user_id:str)->list[Dict[str,Any]]:
        now=time.monotonic(); key=str(user_id); uid=int(user_id or 0); owner=self._is_owner(user_id)
        with self._fast_cache_lock:
            cached=self._fast_groups_cache.get(key)
            if cached and now-cached[0]<4.0: return [dict(x) for x in cached[1]]
        merged: Dict[int,Dict[str,Any]]={}
        try:
            persisted=self._ipc("main",{"op":"official_group_access","action":"list","requester_user_id":uid},timeout=1.5)
        except Exception: persisted={}
        for item in persisted.get("groups") or []:
            if not isinstance(item,dict): continue
            gid=int(item.get("group_id") or 0)
            if gid>0: merged[gid]={"user_id":key,"group_id":gid,"account_key":str(item.get("account_key") or "main"),"title":str(item.get("title") or ""),"member_count":int(item.get("member_count") if item.get("member_count") is not None else -1)}
        from concurrent.futures import ThreadPoolExecutor, as_completed
        def fetch(acc:str):
            try: return acc,self._ipc(acc,{"op":"groups","requester_user_id":uid,"owner_override":owner},timeout=1.4)
            except Exception: return acc,{}
        with ThreadPoolExecutor(max_workers=len(IPC_ACCOUNT_KEYS),thread_name_prefix="zivo-groups") as pool:
            futs=[pool.submit(fetch,a) for a in IPC_ACCOUNT_KEYS]
            for fut in as_completed(futs):
                acc,data=fut.result()
                for item in data.get("groups") or []:
                    if not isinstance(item,dict): continue
                    gid=int(item.get("group_id") or 0)
                    if gid<=0: continue
                    row={"user_id":key,"group_id":gid,"account_key":str(item.get("account_key") or acc),"title":str(item.get("title") or ""),"member_count":int(item.get("member_count") if item.get("member_count") is not None else -1)}
                    merged[gid]=row
                    try: self.store.add_managed_group(user_id=key,group_id=gid,account_key=row["account_key"],title=row["title"],member_count=row["member_count"])
                    except Exception: pass
        for item in self.store.managed_groups(key):
            gid=int(item["group_id"] or 0)
            if gid>0 and gid not in merged: merged[gid]={"user_id":key,"group_id":gid,"account_key":str(item["account_key"] or "main"),"title":str(item["title"] or ""),"member_count":int(item["member_count"] if item["member_count"] is not None else -1)}
        rows=list(merged.values())
        with self._fast_cache_lock: self._fast_groups_cache[key]=(now,[dict(x) for x in rows])
        return rows

    def _premium_status_for_group(self,user_id:str,group_id:int)->Dict[str,Any]:
        key=(str(user_id),int(group_id)); now=time.monotonic()
        with self._fast_cache_lock:
            cached=self._fast_plan_cache.get(key)
            if cached and now-cached[0]<4.0: return dict(cached[1])
        result=super()._premium_status_for_group(user_id,group_id)
        with self._fast_cache_lock: self._fast_plan_cache[key]=(now,dict(result))
        return result

    def _persist_managed_group(self,**kwargs:Any)->None:
        super()._persist_managed_group(**kwargs)
        with self._fast_cache_lock: self._fast_groups_cache.pop(str(kwargs.get("user_id") or ""),None)

    # ---- pretty order helpers ----
    def _order_kind_label18(self,o:Dict[str,Any])->str:
        kind=str(o.get("order_kind") or "subscription")
        return {"subscription":"💎 پنل گروه","meow_purchase":"🐱 خرید Meow","meow_gift_code":"🎁 کد هدیه Meow","wallet_topup":"👛 شارژ کیف پول"}.get(kind,kind)

    def _order_summary18(self,o:Dict[str,Any])->str:
        code=str(o.get("order_code") or o.get("order_id") or "—"); kind=str(o.get("order_kind") or "subscription")
        lines=[f"🔖 کد رهگیری: {code}",f"🛒 محصول: {self._order_kind_label18(o)}",f"💰 مبلغ: {int(o.get('amount_rial') or 0):,} ریال",f"📌 وضعیت: {o.get('status') or '—'}"]
        if kind=="subscription": lines += [f"🏷 گروه: {o.get('group_title') or o.get('group_id') or '—'}",f"💎 پلن: {str(o.get('plan') or '').upper()}",f"📅 مدت: {int(o.get('duration_days') or 0)} روز"]
        elif kind=="meow_purchase": lines += [f"🐱 تعداد: {int(o.get('meow_amount') or 0):,} Meow",f"🎯 گیرنده: {int(o.get('target_user_id') or 0)}"]
        elif kind=="meow_gift_code": lines += [f"🐱 مقدار کد: {int(o.get('meow_amount') or 0):,} Meow",f"🎟 کد: {o.get('gift_code') or 'بعد از تأیید ساخته می‌شود'}"]
        return "\n".join(lines)

    def _card_buttons18(self,oid:int,card:str,rial:int)->Dict[str,Any]:
        return {"inline_keyboard":[
            [{"text":"📋 شماره کارت","callback_data":f"receipt:copycard:{oid}"},{"text":"💵 مبلغ ریال","callback_data":f"receipt:copyamount:{oid}"}],
            [{"text":"✅ پرداخت کردم · ارسال رسید","callback_data":f"receipt:start:{oid}"}],
            [{"text":"❌ لغو سفارش","callback_data":f"prem:cancel:{oid}"},{"text":"🏠 پنل","callback_data":"menu:home"}],
        ]}

    def _remember_card18(self,user_id:str,oid:int,card:str,rial:int)->None:
        self._receipt_state[str(user_id)]={"stage":"card_ready","order_id":int(oid),"card":str(card),"rial":int(rial)}

    def _premium_pay(self,user_id:str,chat_id:str,order_id:int,method:str)->str:
        # The base callback router calls _premium_pay. Official18/19 accidentally
        # added the receipt-ledger UI under _premium_payment, so subscription
        # purchases kept reaching the old support-only card page. Override the
        # method that is actually dispatched and keep every non-card method on
        # the established checkout path.
        if method!="card": return super()._premium_pay(user_id,chat_id,order_id,method)
        try: res=self._ipc("main",{"op":"premium","action":"card","requester_user_id":int(user_id),"order_ref":int(order_id)},timeout=5.0)
        except Exception as exc: res={"ok":False,"error":str(exc)}
        if not res.get("ok") or not res.get("manual_receipt_required"):
            text="🟠 کارت‌به‌کارت الان فعال نیست."; self._send(chat_id,text); return text
        order=res.get("order") or {}; card=str(res.get("card_number") or ""); rial=int(res.get("amount_rial") or order.get("amount_rial") or 0); self._remember_card18(user_id,order_id,card,rial)
        text=("💳 پرداخت کارت‌به‌کارت\n━━━━━━━━━━━━━━━━━━\n"+self._order_summary18(order)+f"\n\n💳 کارت: {card}\n👤 به نام: {res.get('card_holder') or '—'}\n💵 مبلغ دقیق: {rial:,} ریال\n\nبعد از واریز روی «✅ پرداخت کردم · ارسال رسید» بزن. ZIVO فقط عکس رسید را قبول می‌کند.")
        self._send(chat_id,text,reply_markup=self._card_buttons18(order_id,card,rial)); return text

    # Compatibility for the direct regression harness used by Official18/19.
    def _premium_payment(self,user_id:str,chat_id:str,order_id:int,method:str)->str:
        return self._premium_pay(user_id,chat_id,order_id,method)
    ...
    def _meow_card_page(self,user_id:str,chat_id:str,order_id:int)->str:
        ...
        if not res.get("ok"):
            text="🟠 کارت‌به‌کارت الان فعال نیست."; self._send(chat_id,text); return text
        order=res.get("order") or {}; card=str(res.get("card_number") or ""); rial=int(res.get("amount_rial") or order.get("amount_rial") or 0); self._remember_card18(user_id,order_id,card,rial)
        text="💳 کارت‌به‌کارت\n━━━━━━━━━━━━━━━━━━\n"+self._order_summary18(order)+f"\n\n💳 کارت: {card}\n💵 مبلغ دقیق: {rial:,} ریال\n\nبعد از واریز رسید را از مسیر دکمه پایین بفرست."
        self._send(chat_id,text,reply_markup=self._card_buttons18(order_id,card,rial)); return text

    def _receipt_request18(self,user_id:str,chat_id:str,oid:int)->str:
        st=self._receipt_state.setdefault(str(user_id),{}); st.update({"stage":"receipt_photo","order_id":int(oid)})
        text="🧾 ارسال رسید\n━━━━━━━━━━━━━━━━━━\nفقط عکس رسید پرداخت را بفرست.\n\n⛔ متن، ویس، فایل و ویدیو به‌عنوان رسید قبول نمی‌شود.\nبعد از دریافت عکس، سفارش با همان کد رهگیری برای مدیریت ارسال می‌شود."
        self._send(chat_id,text,reply_markup={"inline_keyboard":[[{"text":"❌ لغو ارسال رسید","callback_data":"receipt:cancel"}]]}); return text

    def _admin_order_markup18(self,o:Dict[str,Any])->Dict[str,Any]:
        oid=int(o.get("order_id") or 0); status=str(o.get("status") or ""); buyer=dict(o.get("buyer") or {}); username=str(buyer.get("username") or "").strip().lstrip("@")
        rows=[]
        if status!="activated": rows.append([{"text":"✅ تأیید و فعال‌سازی","callback_data":f"payadmin:approve:{oid}"},{"text":"❌ رد پرداخت","callback_data":f"payadmin:rejectmenu:{oid}"}])
        else: rows.append([{"text":"↩️ برگشت تأیید و کسر سرویس","callback_data":f"payadmin:reverse:{oid}"}])
        if status in {"rejected","reversed"}: rows.append([{"text":"✅ تأیید مجدد","callback_data":f"payadmin:approve:{oid}"}])
        if username: rows.append([{"text":"👤 نمایش پروفایل","url":f"https://splus.ir/{username}"}])
        else: rows.append([{"text":"👤 مشخصات خریدار","callback_data":f"payadmin:user:{oid}"}])
        rows += [[{"text":"🔄 بروزرسانی سفارش","callback_data":f"payadmin:o:{oid}"},{"text":"📚 همه سفارش‌ها","callback_data":"payadmin:list"}],[{"text":"👑 پنل ادمین","callback_data":"admin:home"}]]
        return {"inline_keyboard":rows}

    def _admin_order_detail18(self,user_id:str,chat_id:str,order_ref:Any)->str:
        try: res=self._ipc("main",{"op":"official_payment_admin","action":"detail","requester_user_id":int(user_id),"order_ref":order_ref},timeout=5.0)
        except Exception as exc: res={"ok":False,"error":str(exc)}
        if not res.get("ok"):
            text=f"❌ سفارش پیدا نشد: {res.get('error') or 'NOT_FOUND'}"; self._send(chat_id,text); return text
        o=dict(res.get("order") or {}); buyer=dict(o.get("buyer") or {}); review=dict(o.get("review") or {})
        text=("🧾 پرونده سفارش\n━━━━━━━━━━━━━━━━━━\n"+self._order_summary18(o)+"\n\n"
              f"👤 خریدار: {buyer.get('first_name') or '—'}\n🔗 یوزرنیم: @{str(buyer.get('username') or '—').lstrip('@')}\n🆔 ID: {int(o.get('buyer_user_id') or 0)}\n"
              f"🧑‍⚖️ وضعیت بررسی: {review.get('admin_status') or '—'}\n📝 دلیل/یادداشت: {review.get('admin_reason_text') or o.get('receipt_note') or '—'}")
        fid=str(review.get("receipt_file_id") or o.get("receipt_path") or "").strip()
        if fid and self.transport is not None:
            try: self.transport.send_media(str(chat_id),{"type":"photo","file_id":fid},caption=f"🧾 رسید سفارش {o.get('order_code') or o.get('order_id')}")
            except Exception as exc: log.warning("admin receipt photo send failed | order=%s | %s",o.get('order_id'),exc)
        self._send(chat_id,text,reply_markup=self._admin_order_markup18(o)); return text

    def _admin_orders18(self,user_id:str,chat_id:str)->str:
        try: res=self._ipc("main",{"op":"official_payment_admin","action":"list","requester_user_id":int(user_id),"limit":20},timeout=5.0)
        except Exception as exc: res={"ok":False,"error":str(exc)}
        rows=res.get("orders") or []
        text="🧾 مدیریت سفارش‌ها\n━━━━━━━━━━━━━━━━━━\nآخرین سفارش‌ها؛ برای جست‌وجوی دقیق با کد رهگیری از دکمه پایین استفاده کن."
        kb=[]
        for o in rows[:12]:
            icon={"activated":"✅","receipt_submitted":"🟠","rejected":"❌","reversed":"↩️","awaiting_receipt":"🧾"}.get(str(o.get("status") or ""),"•")
            kb.append([{"text":f"{icon} {o.get('order_code') or o.get('order_id')} · {self._order_kind_label18(o)}","callback_data":f"payadmin:o:{int(o.get('order_id') or 0)}"}])
        kb += [[{"text":"🔎 جست‌وجو با کد رهگیری","callback_data":"payadmin:search"}],[{"text":"👑 پنل ادمین","callback_data":"admin:home"}]]
        self._send(chat_id,text,reply_markup={"inline_keyboard":kb}); return text

    def _notify_owner_receipt18(self,o:Dict[str,Any])->None:
        review=dict(o.get("review") or {}); fid=str(review.get("receipt_file_id") or o.get("receipt_path") or ""); buyer=dict(o.get("buyer") or {})
        caption=f"🧾 رسید جدید · {o.get('order_code') or o.get('order_id')}\n{self._order_kind_label18(o)} · {int(o.get('amount_rial') or 0):,} ریال"
        for owner in GLOBAL_OWNER_IDS:
            try:
                if fid and self.transport is not None: self.transport.send_media(str(owner),{"type":"photo","file_id":fid},caption=caption)
                text=("🔔 پرداخت کارت‌به‌کارت جدید\n━━━━━━━━━━━━━━━━━━\n"+self._order_summary18(o)+f"\n\n👤 {buyer.get('first_name') or '—'} · @{str(buyer.get('username') or '—').lstrip('@')} · {int(o.get('buyer_user_id') or 0)}")
                self._send(str(owner),text,reply_markup=self._admin_order_markup18(o))
            except Exception: log.exception("receipt owner notify failed | order=%s",o.get("order_id"))

    def _reject_menu18(self,chat_id:str,oid:int)->str:
        kb=[[{"text":"💵 مبلغ اشتباه","callback_data":f"payadmin:reject:{oid}:amount"},{"text":"🧾 رسید نامعتبر","callback_data":f"payadmin:reject:{oid}:invalid"}],
            [{"text":"🔍 پرداخت پیدا نشد","callback_data":f"payadmin:reject:{oid}:notfound"},{"text":"♻️ رسید تکراری","callback_data":f"payadmin:reject:{oid}:duplicate"}],
            [{"text":"⚠️ مغایرت اطلاعات","callback_data":f"payadmin:reject:{oid}:mismatch"}],[{"text":"✍️ دلیل دستی / عکس","callback_data":f"payadmin:rejectmanual:{oid}"}],[{"text":"↩️ سفارش","callback_data":f"payadmin:o:{oid}"}]]
        text="❌ دلیل رد پرداخت را انتخاب کن.\nدلیل برای کاربر ارسال و در پرونده سفارش ذخیره می‌شود."; self._send(chat_id,text,reply_markup={"inline_keyboard":kb}); return text

    # ---- admin home ----
    def _admin_home(self,user_id:str,chat_id:str)->str:
        text=("👑 پنل ادمین ZIVO\n━━━━━━━━━━━━━━━━━━\n📣 تبلیغات چندرسانه‌ای\n🧾 رسیدها و سفارش‌ها با کد رهگیری\n👥 Inventory مخاطبان و Campaignها\n\nتمام تصمیم‌های پرداخت داخل Ledger ثبت می‌شوند و تأیید اشتباه قابل برگشت است.")
        markup={"inline_keyboard":[[{"text":"🧾 سفارش‌ها و رسیدها","callback_data":"payadmin:list"},{"text":"📣 تبلیغات","callback_data":"admin:ads"}],
             [{"text":"👥 مخاطبان","callback_data":"admin:audience"},{"text":"🔄 اسکن کامل گروه و پیوی","callback_data":"admin:inventory"}],
             [{"text":"📊 وضعیت ارسال","callback_data":"admin:status"}],[{"text":"🏠 پنل اصلی","callback_data":"menu:home"}]]}
        self._send(chat_id,text,reply_markup=markup); return text

    # ---- Welcome / speaker UX ----
    def _group_control_menu(self,user_id:str)->Dict[str,Any]:
        markup=super()._group_control_menu(user_id); rows=list(markup.get("inline_keyboard") or []); state=self._active_plan_state(user_id); rank=PLAN_RANK.get(str(state.get("plan") or "free"),0)
        insert_at=max(0,len(rows)-3)
        extra=[]
        if rank>=2: extra.append([{"text":"🎨 طراحی خوشامد Gold+","callback_data":"custom:welcome"}])
        if rank>=1: extra.append([{"text":"🧠 آموزش سخنگو","callback_data":"custom:speaker"}])
        rows[insert_at:insert_at]=extra; return {"inline_keyboard":rows}

    def _custom_welcome_menu18(self,user_id:str,chat_id:str)->str:
        text=("🎨 خوشامد حرفه‌ای\n━━━━━━━━━━━━━━━━━━\nمتن اختصاصی با متغیرها:\n`{نام}` `{آیدی}` `{آیدی_عددی}` `{بیو}` `{گروه}` `{ساعت}` `{تاریخ}`\n\nمی‌تونی یک عکس هم برای خوشامد ذخیره کنی. این بخش برای GOLD و DIAMOND فعاله.")
        kb={"inline_keyboard":[[{"text":"✍️ تنظیم متن","callback_data":"custom:welcome:text"},{"text":"🖼 تنظیم عکس","callback_data":"custom:welcome:photo"}],[{"text":"🗑 حذف عکس","callback_data":"custom:welcome:removephoto"}],[{"text":"↩️ مدیریت گروه","callback_data":"menu:control"}]]}
        self._send(chat_id,text,reply_markup=kb); return text

    def _custom_speaker_menu18(self,user_id:str,chat_id:str)->str:
        text=("🧠 آموزش سخنگو\n━━━━━━━━━━━━━━━━━━\nقالب آموزش:\nسلام => سلام، خوش اومدی!\n\n🛡 فیلتر ضد‌فحش Core روی هر دو طرف آموزش فعاله؛ سؤال یا جواب نامناسب ذخیره نمی‌شه.")
        kb={"inline_keyboard":[[{"text":"➕ آموزش جدید","callback_data":"custom:speaker:learn"},{"text":"📚 لیست آموزش‌ها","callback_data":"custom:speaker:list"}],[{"text":"🗑 حذف آموزش","callback_data":"custom:speaker:delete"}],[{"text":"↩️ مدیریت گروه","callback_data":"menu:control"}]]}
        self._send(chat_id,text,reply_markup=kb); return text

    def handle_callback(self,raw:Dict[str,Any])->Optional[str]:
        cb=normalize_callback(raw)
        if cb is None: return super().handle_callback(raw)
        d=str(cb.data or ""); low=d.casefold(); uid=str(cb.sender_id); chat=str(cb.chat_id)
        def ack():
            if self.transport is not None: self.transport.answer_callback(cb.callback_id)
        if low.startswith("receipt:"):
            ack()
            if low=="receipt:cancel": self._receipt_state.pop(uid,None); return self._send_main_menu(chat,uid)
            m=re.fullmatch(r"receipt:start:(\d+)",low)
            if m: return self._receipt_request18(uid,chat,int(m.group(1)))
            m=re.fullmatch(r"receipt:copycard:(\d+)",low)
            if m:
                text=str((self._receipt_state.get(uid) or {}).get("card") or ""); self._send(chat,text); return text
            m=re.fullmatch(r"receipt:copyamount:(\d+)",low)
            if m:
                text=str((self._receipt_state.get(uid) or {}).get("rial") or ""); self._send(chat,text); return text
        if low.startswith("payadmin:") and self._is_owner(uid):
            ack()
            if low=="payadmin:list": return self._admin_orders18(uid,chat)
            if low=="payadmin:search": self._custom_state[uid]={"stage":"order_search"}; text="🔎 کد رهگیری سفارش را بفرست؛ مثل ZV-..."; self._send(chat,text); return text
            m=re.fullmatch(r"payadmin:o:(\d+)",low)
            if m: return self._admin_order_detail18(uid,chat,int(m.group(1)))
            m=re.fullmatch(r"payadmin:user:(\d+)",low)
            if m: return self._admin_order_detail18(uid,chat,int(m.group(1)))
            m=re.fullmatch(r"payadmin:approve:(\d+)",low)
            if m:
                oid=int(m.group(1)); res=self._ipc("main",{"op":"official_payment_admin","action":"approve","requester_user_id":int(uid),"order_id":oid},timeout=8.0)
                if not res.get("ok"): text=f"❌ تأیید نشد: {res.get('error') or 'FAILED'}"; self._send(chat,text); return text
                return self._admin_order_detail18(uid,chat,oid)
            m=re.fullmatch(r"payadmin:reverse:(\d+)",low)
            if m:
                oid=int(m.group(1)); res=self._ipc("main",{"op":"official_payment_admin","action":"reverse","requester_user_id":int(uid),"order_id":oid,"reason_code":"admin-correction","reason_text":"برگشت تأیید توسط مدیریت"},timeout=8.0)
                if not res.get("ok"): text=f"❌ برگشت انجام نشد: {res.get('error') or 'FAILED'}"; self._send(chat,text); return text
                return self._admin_order_detail18(uid,chat,oid)
            m=re.fullmatch(r"payadmin:rejectmenu:(\d+)",low)
            if m: return self._reject_menu18(chat,int(m.group(1)))
            m=re.fullmatch(r"payadmin:reject:(\d+):([a-z]+)",low)
            if m:
                oid=int(m.group(1)); code=m.group(2); reason=self.RECEIPT_REASONS.get(code,"پرداخت رد شد")
                res=self._ipc("main",{"op":"official_payment_admin","action":"reject","requester_user_id":int(uid),"order_id":oid,"reason_code":code,"reason_text":reason},timeout=8.0)
                if not res.get("ok"): self._send(chat,f"❌ رد انجام نشد: {res.get('error') or 'FAILED'}"); return None
                return self._admin_order_detail18(uid,chat,oid)
            m=re.fullmatch(r"payadmin:rejectmanual:(\d+)",low)
            if m:
                self._custom_state[uid]={"stage":"manual_reject","order_id":int(m.group(1))}; text="✍️ دلیل دستی را بفرست. می‌تونی متن، عکس یا عکس با کپشن بفرستی؛ همان پیام برای خریدار ارسال می‌شود."; self._send(chat,text); return text
        if low.startswith("custom:"):
            ack(); active=self._active_group(uid)
            if active is None: return self._send_groups_menu(uid,chat)
            gid=int(active.get("group_id") or 0); account=str(active.get("account_key") or "main")
            if low=="custom:welcome": return self._custom_welcome_menu18(uid,chat)
            if low=="custom:welcome:text": self._custom_state[uid]={"stage":"welcome_text","group_id":gid,"account":account}; text="✍️ متن خوشامد را بفرست. متغیرها قابل استفاده‌اند: {نام} {آیدی} {آیدی_عددی} {بیو} {گروه} {ساعت} {تاریخ}"; self._send(chat,text); return text
            if low=="custom:welcome:photo": self._custom_state[uid]={"stage":"welcome_photo","group_id":gid,"account":account}; text="🖼 فقط یک عکس برای خوشامد بفرست."; self._send(chat,text); return text
            if low=="custom:welcome:removephoto":
                res=self._ipc(account,{"op":"official_group_customization","action":"welcome_media_remove","requester_user_id":int(uid),"group_id":gid},timeout=5.0); text="✅ عکس خوشامد حذف شد." if res.get("ok") else f"❌ {res.get('error') or 'FAILED'}"; self._send(chat,text); return text
            if low=="custom:speaker": return self._custom_speaker_menu18(uid,chat)
            if low=="custom:speaker:learn": self._custom_state[uid]={"stage":"speaker_learn","group_id":gid,"account":account}; text="فرمت آموزش را بفرست:\nسؤال => جواب\n\nعبارت‌های فحش/نامناسب توسط Core رد می‌شوند."; self._send(chat,text); return text
            if low=="custom:speaker:list":
                res=self._ipc(account,{"op":"official_group_customization","action":"speaker_list","requester_user_id":int(uid),"group_id":gid},timeout=5.0); items=res.get("items") or []; text="📚 آموزش‌های سخنگو\n━━━━━━━━━━━━━━━━━━\n"+("\n".join(f"• {x.get('trigger')} ⇒ {x.get('response')}" for x in items[:30]) if items else "هنوز آموزشی ثبت نشده."); self._send(chat,text); return text
            if low=="custom:speaker:delete": self._custom_state[uid]={"stage":"speaker_delete","group_id":gid,"account":account}; text="متن سؤال/Triggerای که باید حذف شود را دقیق بفرست."; self._send(chat,text); return text
        return super().handle_callback(raw)
    ...
    def handle(self,raw:Dict[str,Any])->Optional[str]:
        ...
        chat=safe_id((rawmsg.get("chat") or {}).get("id") if isinstance(rawmsg.get("chat"),dict) else "") or (msg.chat_id if msg else sender)
        body=normalize_text(rawmsg.get("text") or rawmsg.get("caption") or (msg.body if msg else ""))
        # photo-only card receipt
        if sender and chat==sender:
            rs=self._receipt_state.get(sender) or {}
            if str(rs.get("stage") or "")=="receipt_photo":
                media=self._incoming_media(raw)
                if not media or str(media.get("type") or "")!="photo":
                    text="⛔ فقط عکس رسید قابل قبول است. لطفاً تصویر رسید را بفرست."; self._send(chat,text); return text
                oid=int(rs.get("order_id") or 0); fid=str(media.get("file_id") or "")
                res=self._ipc("main",{"op":"official_payment_admin","action":"receipt_submit","requester_user_id":int(next(iter(GLOBAL_OWNER_IDS))),"order_id":oid,"buyer_user_id":int(sender),"file_id":fid,"message_id":int(rawmsg.get('message_id') or 0),"caption":body[:1000]},timeout=6.0)
                if not res.get("ok"): text=f"❌ رسید ثبت نشد: {res.get('error') or 'FAILED'}"; self._send(chat,text); return text
                self._receipt_state.pop(sender,None); o=dict(res.get("order") or {}); self._notify_owner_receipt18(o)
                text=f"✅ رسید دریافت شد\n🔖 کد رهگیری: {o.get('order_code') or oid}\n\nرسید برای مدیریت ارسال شد. نتیجه تأیید یا رد همین‌جا بهت اطلاع داده می‌شه."; self._send(chat,text,reply_markup={"inline_keyboard":[[{"text":"🏠 پنل اصلی","callback_data":"menu:home"}]]}); return text
        # owner order-search/manual-rejection states
        if sender and self._is_owner(sender):
            st=self._custom_state.get(sender) or {}; stage=str(st.get("stage") or "")
            if stage=="order_search":
                self._custom_state.pop(sender,None); return self._admin_order_detail18(sender,chat,body.strip())
            if stage=="manual_reject":
                oid=int(st.get("order_id") or 0); media=self._incoming_media(raw); reason=body.strip() or ("پیام تصویری مدیریت" if media else "رد دستی توسط مدیریت")
                res=self._ipc("main",{"op":"official_payment_admin","action":"reject","requester_user_id":int(sender),"order_id":oid,"reason_code":"manual","reason_text":reason},timeout=8.0)
                o=dict(res.get("order") or {}); buyer=str(int(o.get("buyer_user_id") or 0))
                try:
                    if media and self.transport is not None: self.transport.send_media(buyer,media,caption=f"❌ سفارش {o.get('order_code') or oid} رد شد\n{reason}"[:3500])
                    elif buyer!="0": self._send(buyer,f"❌ سفارش {o.get('order_code') or oid} رد شد\n{reason}"[:3900])
                except Exception: pass
                self._custom_state.pop(sender,None); return self._admin_order_detail18(sender,chat,oid)
        # group custom state
        if sender and chat==sender:
            st=self._custom_state.get(sender) or {}; stage=str(st.get("stage") or "")
            if stage in {"welcome_text","welcome_photo","speaker_learn","speaker_delete"}:
                gid=int(st.get("group_id") or 0); account=str(st.get("account") or "main")
                if stage=="welcome_text":
                    if not body.strip(): self._send(chat,"متن خالی قابل ذخیره نیست."); return None
                    res=self._ipc(account,{"op":"official_group_customization","action":"welcome_text","requester_user_id":int(sender),"group_id":gid,"text":body.strip()},timeout=5.0); self._custom_state.pop(sender,None); text="✅ متن خوشامد ذخیره شد." if res.get("ok") else f"❌ {res.get('error') or 'FAILED'}"; self._send(chat,text); return text
                if stage=="welcome_photo":
                    media=self._incoming_media(raw)
                    if not media or str(media.get("type") or "")!="photo": self._send(chat,"⛔ فقط عکس بفرست."); return None
                    path=Path("/opt/zivo60/welcome_media")/f"official22_{gid}_{int(time.time())}.jpg"
                    try: self.transport.download_media_item(media,path) if self.transport is not None else None
                    except Exception as exc: self._send(chat,f"❌ دانلود عکس ناموفق بود: {exc}"); return None
                    res=self._ipc(account,{"op":"official_group_customization","action":"welcome_media_path","requester_user_id":int(sender),"group_id":gid,"path":str(path)},timeout=5.0); self._custom_state.pop(sender,None); text="✅ عکس خوشامد ذخیره شد." if res.get("ok") else f"❌ {res.get('error') or 'FAILED'}"; self._send(chat,text); return text
                if stage=="speaker_learn":
                    if "=>" not in body: self._send(chat,"فرمت درست: سؤال => جواب"); return None
                    trigger,response=[x.strip() for x in body.split("=>",1)]
                    res=self._ipc(account,{"op":"official_group_customization","action":"speaker_learn","requester_user_id":int(sender),"group_id":gid,"trigger":trigger,"response":response},timeout=5.0)
                    if not res.get("ok"):
                        err=str(res.get("error") or "FAILED"); text="⛔ این آموزش به‌دلیل فحش/عبارت نامناسب ذخیره نشد." if "PROFANITY" in err else f"❌ آموزش ذخیره نشد: {err}"
                    else: self._custom_state.pop(sender,None); text=f"✅ آموزش ذخیره شد. تعداد آموزش‌های گروه: {int(res.get('count') or 0)}"
                    self._send(chat,text); return text
                if stage=="speaker_delete":
                    res=self._ipc(account,{"op":"official_group_customization","action":"speaker_delete","requester_user_id":int(sender),"group_id":gid,"trigger":body.strip()},timeout=5.0); self._custom_state.pop(sender,None); text="✅ آموزش حذف شد." if res.get("removed") else "آموزش پیدا نشد."; self._send(chat,text); return text
        return super().handle(raw)


# ---------------------------------------------------------------------------
# Official19: non-blocking cleanup/control UX. Large cleanup commands are
# enqueued into the account worker and never keep the Bot API event loop waiting
# for a multi-page Soroush purge to finish.
# ---------------------------------------------------------------------------
class BotCore(BotCore):
    ...
    def __init__(self, store: Store, transport: Optional[SoroushOfficialTransport]) -> None:
        super().__init__(store, transport)
        self._last_control_job: Dict[str, Dict[str, Any]] = {}

    @classmethod
    def _long_control_command(cls, command: str) -> bool:
        text = normalize_text(command).strip()
        if not text:
            return False
        # Status/configuration queries remain synchronous. Destructive/history
        # cleanup is queued so Official stays responsive.
        if text.startswith("وضعیت پاکسازی") or text.startswith("پاکسازی خودکار"):
            return False
        return text.startswith(cls.LONG_CONTROL_PREFIXES)
    ...
    def _queue_remote_command(self, *, user_id: str, chat_id: str, command_text: str) -> str:
        ...
        if not command:
            return "فرمان خالی است."
        if not self._long_control_command(command):
            return super()._queue_remote_command(user_id=user_id, chat_id=chat_id, command_text=command_text)
        ...
        try:
            ...
            log.warning("async control enqueue failed | account=%s group=%s | %s: %s", account, row["group_id"], type(exc).__name__, exc)
            return f"❌ عملیات وارد صف نشد: {type(exc).__name__}: {exc}"
        if not result.get("ok"):
            return f"❌ عملیات وارد صف نشد: {result.get('error') or result.get('result_code') or 'FAILED'}"
        job_id = int(result.get("job_id") or 0)
        if job_id <= 0:
            return "❌ شناسه عملیات از Core دریافت نشد."
        self._last_control_job[str(user_id)] = {
            "job_id": job_id,
            "account": account,
            "group_id": int(row["group_id"]),
            "command": command,
        ...
        return (
            "🧹 عملیات پاکسازی شروع شد\n"
            "━━━━━━━━━━━━━━━━━━\n"
            f"🏷 گروه: {row.get('title') or row.get('group_id')}\n"
            f"⚡ اکانت: {account}\n"
            f"🔖 کد عملیات: CL-{job_id}\n\n"
            "پاکسازی داخل Core و به‌صورت صفحه‌ای اجرا می‌شه؛ پنل ZIVO منتظر پایان عملیات قفل نمی‌مونه.\n"
            "نتیجه داخل گروه اعلام می‌شه و از دکمه «وضعیت آخرین عملیات» هم می‌تونی بررسی کنی."
        )

    def _group_control_menu(self, user_id: str) -> Dict[str, Any]:
        markup = super()._group_control_menu(user_id)
        rows = list(markup.get("inline_keyboard") or [])
        job = self._last_control_job.get(str(user_id))
        if job and int(job.get("job_id") or 0) > 0:
            insert_at = max(0, len(rows) - 3)
            rows.insert(insert_at, [{"text": "⏳ وضعیت آخرین عملیات", "callback_data": "ctl:jobstatus"}])
        return {"inline_keyboard": rows}

    def _control_job_status(self, user_id: str, chat_id: str) -> str:
        job = self._last_control_job.get(str(user_id))
        if not job:
            text = "عملیات اخیر ثبت نشده."
            self._send(chat_id, text, reply_markup=self._group_control_menu(user_id))
            return text
        account = str(job.get("account") or "main")
        ...
        try:
            ...
            text = f"❌ وضعیت عملیات دریافت نشد: {type(exc).__name__}: {exc}"
            self._send(chat_id, text, reply_markup=self._group_control_menu(user_id))
            return text
        status = str(result.get("status") or result.get("result_code") or "unknown").lower()
        if status in {"queued", "running"}:
            title = "⏳ عملیات هنوز در حال اجراست"
        elif status == "done":
            title = "✅ عملیات با موفقیت تمام شد"
        else:
            title = "❌ عملیات با خطا تمام شد"
        detail = str(result.get("result_text") or result.get("error") or "")[:1200]
        text = (
            f"{title}\n━━━━━━━━━━━━━━━━━━\n"
            f"🔖 CL-{job_id}\n"
            f"⚡ اکانت: {account}\n"
    ...
    def handle_callback(self, raw: Dict[str, Any]) -> Optional[str]:
        callback = normalize_callback(raw)
        if callback is not None and callback.data.casefold() == "ctl:jobstatus":
            if self.transport is not None:
                self.transport.answer_callback(callback.callback_id)
            return self._control_job_status(callback.sender_id, callback.chat_id)
        return super().handle_callback(raw)


# ---------------------------------------------------------------------------
# Official21: audited Rial wallet commerce for every Official-bot purchase.
# ---------------------------------------------------------------------------
class BotCore(BotCore):
    WALLET_MIN_TOPUP_RIAL = 100_000
    WALLET_MAX_TOPUP_RIAL = 500_000_000
    _FA_DIGITS = str.maketrans("۰۱۲۳۴۵۶۷۸۹٠١٢٣٤٥٦٧٨٩", "01234567890123456789")

    def __init__(self, store: Store, transport: Optional[SoroushOfficialTransport]) -> None:
        super().__init__(store, transport)
        self._wallet_state: Dict[str, Dict[str, Any]] = {}
        self._wallet_admin_state: Dict[str, Dict[str, Any]] = {}

    @classmethod
    def _rial_number(cls, value: Any) -> int:
        raw = str(value or "").translate(cls._FA_DIGITS)
        raw = raw.replace(",", "").replace("٬", "").replace(" ", "").strip()
        return int(raw) if raw.isdigit() else 0

    @staticmethod
    def _wallet_reason_label(reason: Any) -> str:
        key = str(reason or "").strip()
        return {
            "wallet-topup": "شارژ کیف پول",
            "subscription-payment": "خرید اشتراک",
            "meow-purchase-payment": "خرید Meow",
            "meow-gift-payment": "خرید کد هدیه Meow",
            "admin-adjust": "اصلاح مدیریت",
        }.get(key, key or "گردش کیف پول")

    def _main_menu_for(self, user_id: Optional[str] = None) -> Dict[str, Any]:
        markup = super()._main_menu_for(user_id)
        rows = [list(row) for row in markup.get("inline_keyboard") or []]
        wallet_row = [
            {"text": "👛 کیف پول من", "callback_data": "wallet:home"},
            {"text": "➕ شارژ کیف پول", "callback_data": "wallet:topup"},
        ]
        if not any(any(str(btn.get("callback_data") or "").startswith("wallet:") for btn in row) for row in rows):
            insert_at = 1 if user_id is not None and self._is_owner(str(user_id)) else 2
            rows.insert(min(insert_at, len(rows)), wallet_row)
        return {"inline_keyboard": rows}

    def _economy_markup17(self) -> Dict[str, Any]:
        markup = super()._economy_markup17()
        rows = [list(row) for row in markup.get("inline_keyboard") or []]
        rows.insert(max(0, len(rows) - 1), [
            {"text": "👛 کیف پول ریالی", "callback_data": "wallet:home"},
            {"text": "➕ شارژ کیف پول", "callback_data": "wallet:topup"},
        ])
        return {"inline_keyboard": rows}

    def _wallet_info(self, user_id: str, limit: int = 12) -> Dict[str, Any]:
        try:
            return self._ipc("main", {
                "op": "premium", "action": "wallet_info",
                "requester_user_id": int(user_id), "limit": int(limit),
            }, timeout=6.0)
        except Exception as exc:
            return {"ok": False, "error": f"{type(exc).__name__}:{exc}"}

    def _wallet_home(self, user_id: str, chat_id: str, *, limit: int = 10) -> str:
        result = self._wallet_info(user_id, limit)
        if not result.get("ok"):
            text = f"❌ اطلاعات کیف پول در دسترس نیست: {result.get('error') or 'FAILED'}"
            self._send(chat_id, text, reply_markup=self._main_menu_for(user_id))
            return text
        balance = int(result.get("wallet_balance") or 0)
        rows = list(result.get("ledger") or [])
        lines = [
            "👛 کیف پول ریالی ZIVO",
            "━━━━━━━━━━━━━━━━━━",
            f"💰 موجودی: {balance:,} ریال",
            f"💵 معادل: {balance // 10:,} تومان",
            "",
            "از این موجودی می‌تونی اشتراک پنل، Meow و کد هدیه Meow بخری.",
        ]
        if rows:
            ...
            for row in rows[:limit]:
                ...
                lines.append(
                    f"• {sign}{delta:,} ریال · مانده {int(row.get('balance_after') or 0):,}\n"
                    f"  {self._wallet_reason_label(row.get('reason'))}" + (f" · {created}" if created else "")
                )
        else:
            lines.extend(["", "📒 هنوز گردشی برای این کیف پول ثبت نشده."])
        markup = {"inline_keyboard": [
            [{"text": "➕ شارژ کیف پول", "callback_data": "wallet:topup"}],
            [{"text": "🔄 بروزرسانی", "callback_data": "wallet:home"}, {"text": "🧾 خریدهای من", "callback_data": "prem:history"}],
            [{"text": "💎 خرید اشتراک", "callback_data": "prem:groups"}, {"text": "🐱 خرید Meow", "callback_data": "meow:buy"}],
            [{"text": "🏠 پنل اصلی", "callback_data": "menu:home"}],
        ]}
        text = "\n".join(lines)
        self._send(chat_id, text, reply_markup=markup)
    ...
    def _wallet_topup_start(self, user_id: str, chat_id: str) -> str:
        self._wallet_state[str(user_id)] = {"stage": "topup_amount"}
        text = (
            "➕ شارژ کیف پول ZIVO\n"
            "━━━━━━━━━━━━━━━━━━\n"
            "مبلغ شارژ را فقط به ریال بفرست.\n"
            f"حداقل: {self.WALLET_MIN_TOPUP_RIAL:,} ریال\n"
            f"حداکثر: {self.WALLET_MAX_TOPUP_RIAL:,} ریال\n\n"
            "مثال: 500000"
        )
        markup = {"inline_keyboard": [
            [{"text": "۵۰۰ هزار ریال", "callback_data": "wallet:preset:500000"}, {"text": "۱ میلیون ریال", "callback_data": "wallet:preset:1000000"}],
            [{"text": "۵ میلیون ریال", "callback_data": "wallet:preset:5000000"}, {"text": "۱۰ میلیون ریال", "callback_data": "wallet:preset:10000000"}],
            [{"text": "❌ لغو", "callback_data": "wallet:cancel"}],
        ]}
        self._send(chat_id, text, reply_markup=markup)
        return text

    def _wallet_topup_create(self, user_id: str, chat_id: str, amount_rial: int) -> str:
        amount = int(amount_rial or 0)
        if amount < self.WALLET_MIN_TOPUP_RIAL or amount > self.WALLET_MAX_TOPUP_RIAL:
            text = f"❌ مبلغ باید بین {self.WALLET_MIN_TOPUP_RIAL:,} تا {self.WALLET_MAX_TOPUP_RIAL:,} ریال باشد."
            self._send(chat_id, text)
            return text
        try:
            result = self._ipc("main", {
                "op": "premium", "action": "create_wallet_topup",
                "requester_user_id": int(user_id), "amount_rial": amount,
            }, timeout=8.0)
        except Exception as exc:
            result = {"ok": False, "error": f"{type(exc).__name__}:{exc}"}
        if not result.get("ok"):
            text = f"❌ سفارش شارژ ساخته نشد: {result.get('error') or 'FAILED'}"
            self._send(chat_id, text)
            return text
        order = dict(result.get("order") or {})
        oid = int(order.get("order_id") or 0)
        payment = dict(result.get("payment") or {})
        self._wallet_state[str(user_id)] = {"stage": "topup_payment", "order_id": oid, "amount_rial": amount}
        lines = [
            "💳 روش شارژ کیف پول",
            "━━━━━━━━━━━━━━━━━━",
            f"🔖 سفارش: {order.get('order_code') or oid}",
            f"➕ مبلغ شارژ: {amount:,} ریال",
            f"💰 موجودی فعلی: {int(result.get('wallet_balance') or 0):,} ریال",
            "",
            "روش پرداخت را انتخاب کن. زیبال بعد از تأیید درگاه، کیف پول را خودکار شارژ می‌کند؛ کارت‌به‌کارت بعد از بررسی رسید شارژ می‌شود.",
        ]
        buttons: List[List[Dict[str, str]]] = []
        if payment.get("zibal_enabled"):
            buttons.append([{"text": "🌐 پرداخت آنلاین زیبال", "callback_data": f"prem:z:{oid}"}])
        if payment.get("card_enabled"):
            buttons.append([{"text": "💳 کارت‌به‌کارت و ارسال رسید", "callback_data": f"prem:c:{oid}"}])
        if not buttons:
            lines.extend(["", "⛔ در حال حاضر هیچ روش پرداخت فعالی تنظیم نشده است."])
        buttons.extend([
            [{"text": "❌ لغو سفارش", "callback_data": f"prem:cancel:{oid}"}],
            [{"text": "👛 برگشت به کیف پول", "callback_data": "wallet:home"}],
        ])
        text = "\n".join(lines)
        self._send(chat_id, text, reply_markup={"inline_keyboard": buttons})
        return text

    def _wallet_topup_success(self, user_id: str, chat_id: str, order: Dict[str, Any], order_id: int) -> str:
        self._wallet_state.pop(str(user_id), None)
        info = self._wallet_info(user_id, 3)
        balance = int(info.get("wallet_balance") or order.get("wallet_balance_after_rial") or 0)
        amount = int(order.get("amount_rial") or 0)
        text = (
            ...
            f"🔖 سفارش: {order.get('order_code') or order_id}\n"
            f"➕ مبلغ شارژ: {amount:,} ریال\n"
            f"💰 موجودی جدید: {balance:,} ریال\n"
            f"💵 معادل: {balance // 10:,} تومان\n\n"
            "اعتبار همین حالا برای خرید اشتراک پنل، Meow و کد هدیه قابل استفاده است."
        )
        markup = {"inline_keyboard": [
            [{"text": "💎 خرید اشتراک", "callback_data": "prem:groups"}, {"text": "🐱 خرید Meow", "callback_data": "meow:buy"}],
            [{"text": "👛 کیف پول و گردش‌ها", "callback_data": "wallet:home"}],
            [{"text": "🏠 پنل اصلی", "callback_data": "menu:home"}],
        ]}
        self._send(chat_id, text, reply_markup=markup)
        return text
    ...
    def _premium_success_page(self, user_id: str, chat_id: str, order: Dict[str, Any], order_id: int) -> str:
        if str(order.get("order_kind") or "") == "wallet_topup":
            return self._wallet_topup_success(user_id, chat_id, order, order_id)
        return super()._premium_success_page(user_id, chat_id, order, order_id)
    ...
    def _premium_checkout_text(self, order: Dict[str, Any], *, wallet_balance: int = 0) -> str:
        ...
        if kind == "wallet_topup":
            return (
                "💳 سفارش شارژ کیف پول\n"
                "━━━━━━━━━━━━━━━━━━\n"
                f"🔖 سفارش: {order.get('order_code') or order.get('order_id')}\n"
                f"➕ مبلغ شارژ: {int(order.get('amount_rial') or 0):,} ریال\n"
                f"💰 موجودی فعلی: {int(wallet_balance):,} ریال\n"
                f"📌 وضعیت: {self._order_status_fa(str(order.get('status') or ''))}\n\n"
                "شارژ کیف پول فقط با زیبال یا کارت‌به‌کارت انجام می‌شود."
            )
        if kind in {"meow_purchase", "meow_gift_code"}:
            target = int(order.get("target_user_id") or 0)
            destination = "کد هدیه" if kind == "meow_gift_code" else ("حساب خودت" if target == int(order.get("buyer_user_id") or 0) else str(target))
            return (
                "💳 سفارش Meow\n"
                "━━━━━━━━━━━━━━━━━━\n"
                f"🔖 سفارش: {order.get('order_code') or order.get('order_id')}\n"
                ...
                f"🎯 مقصد: {destination}\n"
                f"💰 مبلغ: {int(order.get('amount_rial') or 0):,} ریال\n"
                f"👛 کیف پول: {int(wallet_balance):,} ریال\n"
                f"📌 وضعیت: {self._order_status_fa(str(order.get('status') or ''))}"
            )
        return super()._premium_checkout_text(order, wallet_balance=wallet_balance)

    def _premium_checkout_markup(self, order: Dict[str, Any], *, wallet_balance: int = 0, payment: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        kind = str(order.get("order_kind") or "subscription")
        if kind == "subscription":
            return super()._premium_checkout_markup(order, wallet_balance=wallet_balance, payment=payment)
        oid = int(order.get("order_id") or 0)
        amount = int(order.get("amount_rial") or 0)
        status = str(order.get("status") or "")
        ...
        buttons: List[List[Dict[str, str]]] = []
        if status == "activated":
            buttons.append([{"text": "✅ مشاهده نتیجه", "callback_data": f"prem:check:{oid}"}])
        elif status == "cancelled":
            buttons.append([{"text": "➕ شارژ جدید", "callback_data": "wallet:topup"}] if kind == "wallet_topup" else [{"text": "🐱 خرید جدید", "callback_data": "meow:buy"}])
        else:
            if kind != "wallet_topup" and amount > 0 and wallet_balance >= amount:
                buttons.append([{"text": "👛 پرداخت با کیف پول", "callback_data": f"prem:w:{oid}"}])
            if payment.get("zibal_enabled") or int(order.get("zibal_track_id") or 0) > 0:
                buttons.append([{"text": "🌐 پرداخت آنلاین زیبال", "callback_data": f"prem:z:{oid}"}])
            if payment.get("card_enabled"):
                buttons.append([{"text": "💳 کارت‌به‌کارت و ارسال رسید", "callback_data": f"prem:c:{oid}"}])
            buttons.append([{"text": "🔄 بررسی پرداخت", "callback_data": f"prem:check:{oid}"}])
            buttons.append([{"text": "❌ لغو سفارش", "callback_data": f"prem:cancel:{oid}"}])
        back = "wallet:home" if kind == "wallet_topup" else "menu:economy"
        buttons.append([{"text": "↩️ برگشت", "callback_data": back}, {"text": "🏠 پنل", "callback_data": "menu:home"}])
        return {"inline_keyboard": buttons}

    def _premium_history(self, user_id: str, chat_id: str) -> str:
        try:
            result = self._ipc("main", {
                "op": "premium", "action": "history", "requester_user_id": int(user_id),
            }, timeout=6.0)
        except Exception as exc:
            result = {"ok": False, "error": f"{type(exc).__name__}:{exc}", "orders": []}
        rows = list(result.get("orders") or [])
        lines = ["🧾 خریدها و تراکنش‌های من", "━━━━━━━━━━━━━━━━━━"]
        if not rows:
            lines.append("هنوز سفارشی ثبت نکردی.")
        for order in rows[:15]:
            kind = str(order.get("order_kind") or "subscription")
            if kind == "wallet_topup":
                subject = "شارژ کیف پول"
            elif kind == "meow_purchase":
                subject = f"خرید {int(order.get('meow_amount') or 0):,} Meow"
            elif kind == "meow_gift_code":
                subject = f"کد هدیه {int(order.get('meow_amount') or 0):,} Meow"
            else:
                subject = f"{str(order.get('plan') or '').upper()} · {order.get('group_title') or order.get('group_id')}"
            lines.extend([
                "",
                f"• {order.get('order_code') or order.get('order_id')} · {subject}",
                f"  {int(order.get('amount_rial') or 0):,} ریال · {order.get('method') or 'بدون روش'} · {self._order_status_fa(str(order.get('status') or ''))}",
            ])
        text = "\n".join(lines)
        self._send(chat_id, text, reply_markup={"inline_keyboard": [
            [{"text": "👛 کیف پول", "callback_data": "wallet:home"}, {"text": "💎 خرید اشتراک", "callback_data": "prem:groups"}],
            [{"text": "🐱 خرید Meow", "callback_data": "meow:buy"}],
            [{"text": "🏠 پنل اصلی", "callback_data": "menu:home"}],
        ]})
        return text
    ...
    def _meow_payment_page(self, user_id: str, chat_id: str, order_id: int) -> str:
        try:
            result = self._ipc("main", {
                "op": "premium", "action": "order", "requester_user_id": int(user_id),
                "order_ref": int(order_id),
            }, timeout=6.0)
        except Exception as exc:
            result = {"ok": False, "error": f"{type(exc).__name__}:{exc}"}
        if not result.get("ok"):
            text = f"❌ صفحه پرداخت باز نشد: {result.get('error') or 'FAILED'}"
            self._send(chat_id, text, reply_markup=self._economy_markup17())
            return text
        order = dict(result.get("order") or {})
        if str(order.get("status") or "") == "activated":
            return self._meow_success(user_id, chat_id, order, order_id)
        amount = int(order.get("amount_rial") or 0)
        balance = int(result.get("wallet_balance") or 0)
        payment = dict(result.get("payment") or {})
        lines = [
            ...
            f"🐱 تعداد: {int(order.get('meow_amount') or 0):,} Meow",
            f"💰 مبلغ: {amount:,} ریال",
            f"👛 موجودی کیف پول: {balance:,} ریال",
            "",
            "روش پرداخت را انتخاب کن:",
        ]
        buttons: List[List[Dict[str, str]]] = []
        if amount > 0 and balance >= amount:
            buttons.append([{"text": "👛 پرداخت فوری با کیف پول", "callback_data": f"prem:w:{order_id}"}])
        elif amount > 0:
            lines.append(f"ℹ️ برای پرداخت با کیف پول، {amount - balance:,} ریال کم داری.")
            buttons.append([{"text": "➕ شارژ کیف پول", "callback_data": "wallet:topup"}])
        if payment.get("zibal_enabled") or int(order.get("zibal_track_id") or 0) > 0:
            buttons.append([{"text": "🌐 پرداخت آنلاین زیبال", "callback_data": f"prem:z:{order_id}"}])
        if payment.get("card_enabled"):
            buttons.append([{"text": "💳 کارت‌به‌کارت و ارسال رسید", "callback_data": f"prem:c:{order_id}"}])
        buttons.extend([
            [{"text": "🔄 بررسی پرداخت", "callback_data": f"prem:check:{order_id}"}],
            [{"text": "❌ لغو سفارش", "callback_data": f"meow:order:cancel:{order_id}"}],
        ])
        text = "\n".join(lines)
        self._send(chat_id, text, reply_markup={"inline_keyboard": buttons})
        return text
    ...
    def _premium_pay(self, user_id: str, chat_id: str, order_id: int, method: str) -> str:
        if method == "card":
            return super()._premium_pay(user_id, chat_id, order_id, method)
        action = "wallet_pay" if method == "wallet" else "zibal"
        try:
            result = self._ipc("main", {
                "op": "premium", "action": action, "requester_user_id": int(user_id),
                "order_ref": int(order_id),
            }, timeout=20.0)
        except Exception as exc:
            result = {"ok": False, "error": f"{type(exc).__name__}:{exc}"}
        ...
        if method == "wallet":
            ...
            if error == "INSUFFICIENT_WALLET_BALANCE":
                text = (
                    ...
                    f"موجودی: {int(result.get('wallet_balance') or 0):,} ریال\n"
                    f"مبلغ سفارش: {int(order.get('amount_rial') or 0):,} ریال"
                )
            elif error == "WALLET_TOPUP_CANNOT_USE_WALLET":
                text = "⛔ شارژ کیف پول با موجودی همان کیف پول ممکن نیست. زیبال یا کارت‌به‌کارت را انتخاب کن."
            else:
                text = f"❌ پرداخت کیف پول انجام نشد: {error or 'FAILED'}"
            self._send(chat_id, text, reply_markup={"inline_keyboard": [
                [{"text": "➕ شارژ کیف پول", "callback_data": "wallet:topup"}],
                [{"text": "👛 کیف پول من", "callback_data": "wallet:home"}],
            ]})
            return text
        payment_url = str(result.get("payment_url") or "")
        if not result.get("ok") or not payment_url:
            text = f"❌ درگاه زیبال آماده نشد: {result.get('error') or 'PAYMENT_URL_UNAVAILABLE'}"
            self._send(chat_id, text)
            return text
        self._watch_payment(user_id, chat_id, order_id)
        text = (
            "🌐 درگاه زیبال آماده شد\n"
            "━━━━━━━━━━━━━━━━━━\n"
            f"🔖 سفارش: {order.get('order_code') or order_id}\n"
            f"💰 مبلغ دقیق: {int(order.get('amount_rial') or 0):,} ریال\n\n"
            "بعد از پرداخت، ZIVO نتیجه را خودکار تأیید و همان محصول یا اعتبار را فعال می‌کند."
        )
        self._send(chat_id, text, reply_markup={"inline_keyboard": [
            [{"text": "🌐 ورود به درگاه زیبال", "url": payment_url}],
            [{"text": "🔄 بررسی پرداخت", "callback_data": f"prem:check:{order_id}"}],
            [{"text": "🏠 پنل اصلی", "callback_data": "menu:home"}],
        ]})
        return text

    # ----- owner wallet administration -----
    def _wallet_admin_home(self, user_id: str, chat_id: str) -> str:
        try:
            result = self._ipc("main", {
                "op": "official_payment_admin", "action": "wallet_summary",
                "requester_user_id": int(user_id),
            }, timeout=6.0)
        except Exception as exc:
            result = {"ok": False, "error": f"{type(exc).__name__}:{exc}"}
        ...
        text = (
            "👛 مدیریت کیف پول‌های ZIVO\n"
            "━━━━━━━━━━━━━━━━━━\n"
            f"👥 کل کیف پول‌ها: {int(result.get('wallet_count') or 0):,}\n"
            f"✅ دارای موجودی: {int(summary.get('wallet_users') or 0):,}\n"
            f"💰 مجموع موجودی: {int(summary.get('wallet_total_rial') or 0):,} ریال\n"
            f"➕ شارژهای موفق: {int(summary.get('wallet_topups') or 0):,}\n\n"
            "هر افزایش یا کسر با دلیل، موجودی قبل/بعد و شناسه ادمین در دفترکل ثبت می‌شود؛ کسر بیشتر از موجودی رد خواهد شد."
        )
        markup = {"inline_keyboard": [
            [{"text": "➕ افزودن موجودی", "callback_data": "walletadmin:add"}, {"text": "➖ کسر موجودی", "callback_data": "walletadmin:minus"}],
            [{"text": "🔎 جست‌وجوی کاربر", "callback_data": "walletadmin:search"}, {"text": "📋 فهرست کیف پول‌ها", "callback_data": "walletadmin:list"}],
            [{"text": "🔄 بروزرسانی", "callback_data": "walletadmin:home"}],
            [{"text": "👑 پنل ادمین", "callback_data": "admin:home"}],
        ]}
        self._send(chat_id, text, reply_markup=markup)
        return text

    @staticmethod
    def _wallet_admin_identity(wallet: Dict[str, Any]) -> str:
        user = dict(wallet.get("user") or {})
        uid = int(wallet.get("user_id") or user.get("user_id") or 0)
        name = str(user.get("first_name") or "").strip() or "بدون نام"
        username = str(user.get("username") or "").strip().lstrip("@")
        return f"{name}" + (f" · @{username}" if username else "") + f" · {uid}"

    def _wallet_admin_list(self, user_id: str, chat_id: str, offset: int = 0) -> str:
        try:
            result = self._ipc("main", {
                "op": "official_payment_admin", "action": "wallet_list",
                "requester_user_id": int(user_id), "limit": 20, "offset": max(0, int(offset)),
            }, timeout=6.0)
        except Exception as exc:
            result = {"ok": False, "error": f"{type(exc).__name__}:{exc}"}
        wallets = list(result.get("wallets") or [])
        total = int(result.get("wallet_count") or 0)
        lines = ["📋 فهرست کیف پول‌ها", "━━━━━━━━━━━━━━━━━━", f"تعداد کل: {total:,}", ""]
        buttons: List[List[Dict[str, str]]] = []
        for wallet in wallets:
            uid = int(wallet.get("user_id") or 0)
            name = str(wallet.get("first_name") or "").strip() or str(uid)
            username = str(wallet.get("username") or "").strip().lstrip("@")
            label = name + (f" · @{username}" if username else "")
            balance = int(wallet.get("balance_rial") or 0)
            lines.append(f"• {label} · {uid} · {balance:,} ریال")
            buttons.append([{"text": f"👤 {label[:25]} · {balance:,}", "callback_data": f"walletadmin:u:{uid}"}])
        if not wallets:
            lines.append("هنوز کیف پولی ثبت نشده است.")
        nav: List[Dict[str, str]] = []
        if offset > 0:
            nav.append({"text": "⬅️ قبلی", "callback_data": f"walletadmin:list:{max(0, offset - 20)}"})
        if offset + len(wallets) < total:
            nav.append({"text": "بعدی ➡️", "callback_data": f"walletadmin:list:{offset + 20}"})
        if nav:
            buttons.append(nav)
        buttons.extend([
            [{"text": "🔎 جست‌وجو", "callback_data": "walletadmin:search"}],
            [{"text": "👛 مدیریت کیف پول", "callback_data": "walletadmin:home"}],
        ])
        text = "\n".join(lines)
        self._send(chat_id, text, reply_markup={"inline_keyboard": buttons})
        return text

    def _wallet_admin_fetch(self, user_id: str, reference: Any) -> Dict[str, Any]:
        try:
            return self._ipc("main", {
                "op": "official_payment_admin", "action": "wallet_detail",
                "requester_user_id": int(user_id), "reference": reference, "limit": 15,
            }, timeout=6.0)
        except Exception as exc:
            return {"ok": False, "error": f"{type(exc).__name__}:{exc}"}

    def _wallet_admin_detail(self, user_id: str, chat_id: str, reference: Any) -> str:
        result = self._wallet_admin_fetch(user_id, reference)
        if not result.get("ok"):
            text = f"❌ کاربر/کیف پول پیدا نشد: {result.get('error') or 'NOT_FOUND'}"
            self._send(chat_id, text)
            return text
        wallet = dict(result.get("wallet") or {})
        user = dict(wallet.get("user") or {})
        uid = int(wallet.get("user_id") or user.get("user_id") or 0)
        name = str(user.get("first_name") or "").strip() or "—"
        username = str(user.get("username") or "").strip().lstrip("@")
        lines = [
            "👤 پرونده کیف پول",
            "━━━━━━━━━━━━━━━━━━",
            ...
            f"User ID: {uid}",
            f"شروع بات: {'بله' if user.get('seen') else 'خیر'}",
            f"عضویت تأییدشده: {'بله' if user.get('membership_passed') else 'خیر'}",
            f"آخرین حضور: {str(user.get('last_seen_at') or '—')[:25]}",
            f"💰 موجودی: {int(wallet.get('balance_rial') or 0):,} ریال",
        ]
        ...
        if ledger:
            ...
            for row in ledger[:10]:
                delta = int(row.get("delta_rial") or 0)
                lines.append(
                    f"• {delta:+,} · مانده {int(row.get('balance_after') or 0):,} · "
                    f"{self._wallet_reason_label(row.get('reason'))} · {str(row.get('source') or '—')}"
                )
        else:
            lines.extend(["", "📒 گردش مالی ثبت نشده است."])
        markup = {"inline_keyboard": [
            [{"text": "➕ افزودن", "callback_data": f"walletadmin:add:{uid}"}, {"text": "➖ کسر", "callback_data": f"walletadmin:minus:{uid}"}],
            [{"text": "🔄 بروزرسانی", "callback_data": f"walletadmin:u:{uid}"}],
            [{"text": "📋 فهرست", "callback_data": "walletadmin:list"}, {"text": "👛 کیف پول‌ها", "callback_data": "walletadmin:home"}],
        ]}
        text = "\n".join(lines)
        self._send(chat_id, text, reply_markup=markup)
        return text
    ...
    def _admin_home(self, user_id: str, chat_id: str) -> str:
        if not self._is_owner(user_id):
            text = "این بخش فقط برای مالک اصلی ZIVO است."
            self._send(chat_id, text)
            return text
        text = (
            "👑 پنل ادمین ZIVO\n"
            "━━━━━━━━━━━━━━━━━━\n"
            "👛 مدیریت کامل کیف پول و دفترکل\n"
            "🧾 رسیدها، تأیید/رد با دلیل و برگشت تصمیم\n"
            "📣 تبلیغات چندرسانه‌ای برای Official، گروه‌ها و پیوی اکانت‌ها\n"
            "👥 Inventory مخاطبان و Campaignها"
        )
        markup = {"inline_keyboard": [
            [{"text": "👛 کیف پول کاربران", "callback_data": "walletadmin:home"}, {"text": "🧾 سفارش‌ها و رسیدها", "callback_data": "payadmin:list"}],
            [{"text": "📣 تبلیغات", "callback_data": "admin:ads"}, {"text": "👥 مخاطبان", "callback_data": "admin:audience"}],
            [{"text": "🔄 اسکن کامل گروه و پیوی", "callback_data": "admin:inventory"}],
            [{"text": "📊 وضعیت ارسال", "callback_data": "admin:status"}],
            [{"text": "🏠 پنل اصلی", "callback_data": "menu:home"}],
        ]}
        self._send(chat_id, text, reply_markup=markup)
        return text
    ...
    def handle_callback(self, raw: Dict[str, Any]) -> Optional[str]:
        ...
        if low.startswith("wallet:"):
            if self.transport is not None:
                self.transport.answer_callback(callback.callback_id)
            if low == "wallet:home":
                self._wallet_state.pop(user_id, None)
                return self._wallet_home(user_id, chat_id)
            if low == "wallet:topup":
                return self._wallet_topup_start(user_id, chat_id)
            if low == "wallet:cancel":
                self._wallet_state.pop(user_id, None)
                return self._wallet_home(user_id, chat_id)
            match = re.fullmatch(r"wallet:preset:(\d+)", low)
            if match:
                return self._wallet_topup_create(user_id, chat_id, int(match.group(1)))
            return None

        if low.startswith("walletadmin:"):
            if self.transport is not None:
                self.transport.answer_callback(callback.callback_id)
            if not self._is_owner(user_id):
                text = "این بخش فقط برای مالک اصلی ZIVO است."
                self._send(chat_id, text)
                return text
            if low == "walletadmin:home":
                self._wallet_admin_state.pop(user_id, None)
                return self._wallet_admin_home(user_id, chat_id)
            if low == "walletadmin:list":
                return self._wallet_admin_list(user_id, chat_id, 0)
            match = re.fullmatch(r"walletadmin:list:(\d+)", low)
            if match:
                return self._wallet_admin_list(user_id, chat_id, int(match.group(1)))
            if low in {"walletadmin:search", "walletadmin:add", "walletadmin:minus"}:
                mode = {"walletadmin:search": "lookup", "walletadmin:add": "add", "walletadmin:minus": "minus"}[low]
                self._wallet_admin_state[user_id] = {"stage": "target", "mode": mode}
                text = "👤 آیدی عددی یا @username کاربر Official ZIVO را بفرست."
                self._send(chat_id, text, reply_markup={"inline_keyboard": [[{"text": "❌ لغو", "callback_data": "walletadmin:home"}]]})
                return text
            match = re.fullmatch(r"walletadmin:u:(\d+)", low)
            if match:
                return self._wallet_admin_detail(user_id, chat_id, int(match.group(1)))
            match = re.fullmatch(r"walletadmin:(add|minus):(\d+)", low)
            if match:
                mode, uid = match.group(1), int(match.group(2))
                result = self._wallet_admin_fetch(user_id, uid)
                if not result.get("ok"):
                    text = f"❌ کاربر پیدا نشد: {result.get('error') or 'NOT_FOUND'}"
                    self._send(chat_id, text)
                    return text
                wallet = dict(result.get("wallet") or {})
                self._wallet_admin_state[user_id] = {"stage": "amount", "mode": mode, "wallet": wallet}
                text = f"{'➕ مبلغ افزایش' if mode == 'add' else '➖ مبلغ کسر'} را فقط به ریال بفرست.\n\n{self._wallet_admin_identity(wallet)}\nموجودی: {int(wallet.get('balance_rial') or 0):,} ریال"
                self._send(chat_id, text, reply_markup={"inline_keyboard": [[{"text": "❌ لغو", "callback_data": "walletadmin:home"}]]})
                return text
            if low == "walletadmin:confirm":
                state = dict(self._wallet_admin_state.get(user_id) or {})
                wallet = dict(state.get("wallet") or {})
                uid = int(wallet.get("user_id") or 0)
                delta = int(state.get("delta_rial") or 0)
                reason = str(state.get("reason") or "").strip()
                if uid <= 0 or delta == 0 or not reason:
                    text = "❌ درخواست تغییر کیف پول ناقص یا منقضی شده است."
                    self._send(chat_id, text)
                    return text
                try:
                    result = self._ipc("main", {
                        "op": "official_payment_admin", "action": "wallet_adjust",
                        "requester_user_id": int(user_id), "reference": uid,
                        "delta_rial": delta, "reason": reason,
                    }, timeout=8.0)
                except Exception as exc:
                    result = {"ok": False, "error": f"{type(exc).__name__}:{exc}"}
                if not result.get("ok"):
                    error = str(result.get("error") or "FAILED")
                    text = "❌ موجودی برای این کسر کافی نیست؛ هیچ تغییری ثبت نشد." if error == "INSUFFICIENT_WALLET_BALANCE" else f"❌ تغییر انجام نشد: {error}"
                    self._send(chat_id, text, reply_markup={"inline_keyboard": [[{"text": "👛 مدیریت کیف پول", "callback_data": "walletadmin:home"}]]})
                    return text
                adjustment = dict(result.get("adjustment") or {})
                self._wallet_admin_state.pop(user_id, None)
                notice = (
                    "💳 کیف پول ZIVO توسط مدیریت به‌روزرسانی شد\n"
                    f"تغییر: {int(adjustment.get('actual_delta_rial') or 0):+,} ریال\n"
                    f"موجودی جدید: {int(adjustment.get('balance_after_rial') or 0):,} ریال\n"
                    f"دلیل: {reason}"
                )
                try:
                    self._send(str(uid), notice, reply_markup={"inline_keyboard": [[{"text": "👛 کیف پول من", "callback_data": "wallet:home"}]]})
                except Exception:
                    pass
                text = (
                    "✅ تغییر کیف پول ثبت شد\n"
                    f"کاربر: {uid}\n"
                    f"تغییر واقعی: {int(adjustment.get('actual_delta_rial') or 0):+,} ریال\n"
                    f"قبل: {int(adjustment.get('balance_before_rial') or 0):,} ریال\n"
                    f"بعد: {int(adjustment.get('balance_after_rial') or 0):,} ریال\n"
                    f"دلیل: {reason}"
                )
                self._send(chat_id, text, reply_markup={"inline_keyboard": [[{"text": "👤 پرونده به‌روز", "callback_data": f"walletadmin:u:{uid}"}], [{"text": "👛 مدیریت کیف پول", "callback_data": "walletadmin:home"}]]})
                return text
            return None
    ...
    def handle(self, raw: Dict[str, Any]) -> Optional[str]:
        ...
        sender = safe_id((message.get("from") or {}).get("id") if isinstance(message.get("from"), dict) else "")
        chat_id = safe_id((message.get("chat") or {}).get("id") if isinstance(message.get("chat"), dict) else "") or sender
        body = normalize_text(message.get("text") or message.get("caption") or "").strip()
        low = body.casefold().replace("‌", " ").strip()

        if sender and chat_id == sender and not self._is_owner(sender) and self.store.gate_pending(sender):
            return super().handle(raw)

        if sender and chat_id == sender and self._is_owner(sender):
            state = dict(self._wallet_admin_state.get(sender) or {})
            stage = str(state.get("stage") or "")
            if stage == "target":
                result = self._wallet_admin_fetch(sender, body)
                if not result.get("ok"):
                    text = "❌ کاربر پیدا نشد. آیدی عددی یا @username کسی را بفرست که Official ZIVO را Start کرده باشد."
                    self._send(chat_id, text)
                    return text
                wallet = dict(result.get("wallet") or {})
                if str(state.get("mode") or "") == "lookup":
                    self._wallet_admin_state.pop(sender, None)
                    return self._wallet_admin_detail(sender, chat_id, int(wallet.get("user_id") or 0))
                state.update({"stage": "amount", "wallet": wallet})
                self._wallet_admin_state[sender] = state
                text = f"✅ {self._wallet_admin_identity(wallet)}\nموجودی: {int(wallet.get('balance_rial') or 0):,} ریال\n\nحالا مبلغ را فقط به ریال بفرست."
                self._send(chat_id, text)
                return text
            if stage == "amount":
                amount = self._rial_number(body)
                if amount <= 0 or amount > 100_000_000_000:
                    text = "❌ مبلغ معتبر و مثبت را فقط به ریال بفرست."
                    self._send(chat_id, text)
                    return text
                wallet = dict(state.get("wallet") or {})
                balance = int(wallet.get("balance_rial") or 0)
                mode = str(state.get("mode") or "")
                if mode == "minus" and amount > balance:
                    text = f"❌ موجودی کافی نیست.\nموجودی: {balance:,} ریال\nکسر درخواستی: {amount:,} ریال"
                    self._send(chat_id, text)
                    return text
                state.update({"stage": "reason", "delta_rial": amount if mode == "add" else -amount})
                self._wallet_admin_state[sender] = state
                text = "📝 دلیل این تغییر را بفرست. دلیل در دفترکل و پیام کاربر ثبت می‌شود."
                self._send(chat_id, text)
                return text
            if stage == "reason":
                reason = body.strip()
                if len(reason) < 3 or len(reason) > 200:
                    text = "❌ دلیل باید بین ۳ تا ۲۰۰ نویسه باشد."
                    self._send(chat_id, text)
                    return text
                wallet = dict(state.get("wallet") or {})
                balance = int(wallet.get("balance_rial") or 0)
                delta = int(state.get("delta_rial") or 0)
                state.update({"stage": "confirm", "reason": reason})
                self._wallet_admin_state[sender] = state
                text = (
                    "⚠️ تأیید نهایی تغییر کیف پول\n"
                    "━━━━━━━━━━━━━━━━━━\n"
                    f"👤 {self._wallet_admin_identity(wallet)}\n"
                    f"💰 موجودی فعلی: {balance:,} ریال\n"
                    f"🔁 تغییر: {delta:+,} ریال\n"
                    f"✅ موجودی بعد: {balance + delta:,} ریال\n"
                    f"📝 دلیل: {reason}"
                )
                self._send(chat_id, text, reply_markup={"inline_keyboard": [[{"text": "✅ ثبت تغییر", "callback_data": "walletadmin:confirm"}], [{"text": "❌ لغو", "callback_data": "walletadmin:home"}]]})
                return text

        if sender and chat_id == sender:
            state = dict(self._wallet_state.get(sender) or {})
            if str(state.get("stage") or "") == "topup_amount":
                if low in {"لغو", "cancel"}:
                    self._wallet_state.pop(sender, None)
                    return self._wallet_home(sender, chat_id)
                amount = self._rial_number(body)
                if amount < self.WALLET_MIN_TOPUP_RIAL or amount > self.WALLET_MAX_TOPUP_RIAL:
                    text = f"❌ مبلغ شارژ باید بین {self.WALLET_MIN_TOPUP_RIAL:,} تا {self.WALLET_MAX_TOPUP_RIAL:,} ریال باشد."
                    self._send(chat_id, text)
                    return text
                return self._wallet_topup_create(sender, chat_id, amount)
            if low in {"کیف پول", "کیف پول من", "موجودی کیف پول", "/wallet"}:
                try:
                    self._remember_official_identity(raw, sender)
                except Exception:
                    pass
                return self._wallet_home(sender, chat_id)
            if low in {"شارژ کیف پول", "افزایش موجودی", "/topup"}:
                try:
                    self._remember_official_identity(raw, sender)
                except Exception:
                    pass
                return self._wallet_topup_start(sender, chat_id)

        return super().handle(raw)


# ---------------------------------------------------------------------------
# Official22: durable Official-start identity sync and stale-recipient repair.
# ---------------------------------------------------------------------------
class BotCore(BotCore):
    OFFICIAL_IDENTITY_SYNC_TTL_SECONDS = 60.0

    def __init__(self, store: Store, transport: Optional[SoroushOfficialTransport]) -> None:
        super().__init__(store, transport)
        self._official_identity_sync_hot: Dict[str, Tuple[float, str, str]] = {}
        self._last_target_resolution_error = ""

    @staticmethod
    def _positive_numeric_id(value: Any) -> str:
        raw = safe_id(value).lstrip("+")
        if not raw.isdigit():
            return ""
        number = int(raw)
    ...
    @classmethod
    def _actor_profile(cls, obj: Any, *, generic_id: bool = True) -> Dict[str, str]:
        if not isinstance(obj, dict):
            return {}
        id_keys = ("id", "user_id", "userId", "sender_id", "senderId", "from_id", "fromId") if generic_id else (
            "user_id", "userId", "sender_id", "senderId", "from_id", "fromId",
        )
        uid = ""
        for key in id_keys:
            uid = cls._positive_numeric_id(obj.get(key))
            if uid:
                break
        username = str(obj.get("username") or obj.get("user_name") or obj.get("userName") or "").strip().lstrip("@")
        first_name = str(obj.get("first_name") or obj.get("firstName") or obj.get("name") or obj.get("display_name") or "").strip()
        if not uid and not username:
            return {}
        return {"id": uid, "username": username[:64], "first_name": first_name[:120]}

    @classmethod
    def _forwarded_user_profile(cls, raw: Dict[str, Any]) -> Dict[str, str]:
        """Extract only the forwarded actor, never the requester/chat actor."""
        msg = cls._raw_message(raw)
        roots: List[Tuple[Any, bool]] = []
        for key in ("forward_from", "forwardFrom", "forwarded_from", "forwardedFrom"):
            if key in msg:
                roots.append((msg.get(key), True))
        for key in ("forward", "forward_info", "forwardInfo", "forward_origin", "forwardOrigin"):
            if key in msg:
                roots.append((msg.get(key), False))
        for key in ("forward_from_id", "forwardFromId", "forwarded_user_id", "forwardedUserId"):
            uid = cls._positive_numeric_id(msg.get(key))
            if uid:
                return {"id": uid, "username": "", "first_name": ""}

        actor_keys = ("sender_user", "senderUser", "from", "sender", "user", "author", "owner")
        nested_keys = actor_keys + (
            "origin", "source", "message", "forward", "forward_info", "forwardInfo",
            "forward_from", "forwardFrom", "forwarded_from", "forwardedFrom",
            "forward_origin", "forwardOrigin",
        )
        stack: List[Tuple[Any, bool, int]] = [(value, actor, 0) for value, actor in roots]
        seen: set[int] = set()
        while stack:
            current, actor_hint, depth = stack.pop(0)
            if depth > 5:
                continue
            if isinstance(current, list):
                stack[0:0] = [(value, actor_hint, depth + 1) for value in current[:12]]
                continue
            if not isinstance(current, dict) or id(current) in seen:
                continue
            seen.add(id(current))
            profile = cls._actor_profile(current, generic_id=actor_hint)
            if profile:
                return profile
            for key in nested_keys:
                value = current.get(key)
                if isinstance(value, (dict, list)):
                    stack.append((value, key in actor_keys, depth + 1))
        return {}

    @classmethod
    def _forwarded_user_reference(cls, raw: Dict[str, Any]) -> str:
        profile = cls._forwarded_user_profile(raw)
        uid = cls._positive_numeric_id(profile.get("id"))
        if uid:
            return uid
        username = str(profile.get("username") or "").strip().lstrip("@")
        return "@" + username if username else ""

    def _sync_official_identity(self, user_id: Any, meta: Dict[str, Any], *, force: bool = False) -> bool:
        uid = self._positive_numeric_id(user_id)
        if not uid:
            return False
        username = str(meta.get("username") or "").strip().lstrip("@")[:64]
        first_name = str(meta.get("first_name") or meta.get("firstName") or meta.get("name") or "").strip()[:120]
        now = time.monotonic()
        previous = self._official_identity_sync_hot.get(uid)
        if not force and previous is not None:
            stamp, old_username, old_first_name = previous
            if now - float(stamp) < self.OFFICIAL_IDENTITY_SYNC_TTL_SECONDS and username == old_username and first_name == old_first_name:
                return True
        try:
            result = self._ipc("main", {
                "op": "official_gate", "action": "seen", "requester_user_id": int(uid),
                "username": username, "first_name": first_name,
            }, timeout=4.0)
        except Exception as exc:
            log.warning("official identity sync failed | user=%s | %s: %s", uid, type(exc).__name__, exc)
            return False
        if not result.get("ok"):
            return False
        self._official_identity_sync_hot[uid] = (now, username, first_name)
        return True

    def _remember_official_identity(self, raw: Dict[str, Any], user_id: str) -> None:
        self._sync_official_identity(user_id, self._sender_meta(raw))

    def _sync_incoming_message_identity(self, raw: Dict[str, Any]) -> None:
        message = self._raw_message(raw)
        actor = message.get("from") if isinstance(message.get("from"), dict) else {}
        chat = message.get("chat") if isinstance(message.get("chat"), dict) else {}
        uid = self._positive_numeric_id(actor.get("id"))
        chat_id = self._positive_numeric_id(chat.get("id")) or uid
        if uid and chat_id == uid:
            self._sync_official_identity(uid, self._actor_profile(actor))

    def _official_identity_is_known(self, user_id: Any) -> bool:
        uid = self._positive_numeric_id(user_id)
        if not uid:
            return False
        if uid in self._official_identity_sync_hot:
            return True
        try:
            result = self._ipc("main", {
                "op": "official_gate", "action": "status", "requester_user_id": int(uid),
            }, timeout=4.0)
        except Exception:
            return False
        state = result.get("state") or {} if result.get("ok") else {}
        return bool(state.get("seen"))

    def _sync_incoming_callback_identity(self, raw: Dict[str, Any]) -> None:
        callback = raw.get("callback_query") if isinstance(raw, dict) and isinstance(raw.get("callback_query"), dict) else {}
    ...
    def _retry_target_lookup(self, requester_user_id: str, target_id: str) -> Dict[str, Any]:
        try:
            return self._ipc("main", {
                "op": "meow_commerce", "action": "resolve_target",
                "requester_user_id": int(requester_user_id), "reference": target_id,
            }, timeout=6.0)
        except Exception as exc:
            return {"ok": False, "error": f"{type(exc).__name__}:{exc}"}

    def _recover_started_target(self, requester_user_id: str, target_id: str, profile: Dict[str, Any]) -> Dict[str, Any]:
        """Repair a stale Core registry only after Official can reach the user."""
        try:
            contact = self.store.contact(target_id)
        except Exception:
            contact = None
        if contact is not None:
            local_meta = {
                "username": str(contact["username"] or "") if "username" in contact.keys() else "",
                "first_name": str(contact["first_name"] or "") if "first_name" in contact.keys() else "",
            }
            merged = dict(local_meta)
            merged.update({key: value for key, value in profile.items() if value})
            if self._sync_official_identity(target_id, merged, force=True):
                return self._retry_target_lookup(requester_user_id, target_id)

        if self.transport is None:
            return {"ok": False, "error": "OFFICIAL_START_CHECK_UNAVAILABLE"}
        try:
            self.transport.send_text(
                target_id,
                "✅ حساب Official ZIVO شما برای دریافت Meow همگام شد. این پیام فقط بررسی آمادگی حساب است و هنوز هیچ Meow کم یا زیاد نشده.",
            )
        except Exception as exc:
            log.info("official target reachability rejected | target=%s | %s: %s", target_id, type(exc).__name__, exc)
            return {"ok": False, "error": "OFFICIAL_USER_NOT_STARTED"}
        if not self._sync_official_identity(target_id, profile, force=True):
            return {"ok": False, "error": "OFFICIAL_START_SYNC_FAILED"}
        recovered = self._retry_target_lookup(requester_user_id, target_id)
        if recovered.get("ok"):
            recovered["registry_recovered"] = True
        return recovered

    def _resolve_target_from_raw(self, user_id: str, raw: Dict[str, Any], body: str) -> Dict[str, Any]:
        profile = self._forwarded_user_profile(raw)
        ref = self._forwarded_user_reference(raw) or normalize_text(body).strip()
        if not ref:
            self._last_target_resolution_error = "TARGET_REFERENCE_EMPTY"
            return {"ok": False, "error": self._last_target_resolution_error}
        try:
            result = self._ipc("main", {
                "op": "meow_commerce", "action": "resolve_target",
                "requester_user_id": int(user_id), "reference": ref,
            }, timeout=6.0)
        except Exception as exc:
            result = {"ok": False, "error": f"{type(exc).__name__}:{exc}"}
        if result.get("ok"):
            self._last_target_resolution_error = ""
            return result
        error = str(result.get("error") or "TARGET_RESOLVE_FAILED")
        target_id = self._positive_numeric_id(profile.get("id")) or self._positive_numeric_id(ref)
        if error == "OFFICIAL_USER_NOT_STARTED" and target_id:
            result = self._recover_started_target(user_id, target_id, profile)
            error = str(result.get("error") or "")
        elif error == "OFFICIAL_USER_NOT_STARTED" and str(ref).startswith("@"):
            error = "OFFICIAL_USERNAME_UNKNOWN"
            result = {"ok": False, "error": error}
        self._last_target_resolution_error = "" if result.get("ok") else error
        return result

    def _official_not_started_text(self) -> str:
        error = str(self._last_target_resolution_error or "OFFICIAL_USER_NOT_STARTED")
        if error == "TARGET_REFERENCE_EMPTY":
            return "❌ مقصد خوانده نشد. آیدی عددی، @username یا خودِ پیام کاربر را Forward کن."
        if error == "OFFICIAL_USERNAME_UNKNOWN":
            return ("❌ این @username هنوز در دفتر Official ثبت نشده.\n\n"
                    "پیام کاربر را Forward کن یا آیدی عددی‌اش را بفرست؛ همچنین خودش یک پیام یا دکمه داخل بات رسمی ZIVO بزند تا username هم ثبت شود.")
        if error in {"OFFICIAL_START_CHECK_UNAVAILABLE", "OFFICIAL_START_SYNC_FAILED"} or ":" in error:
            return "⚠️ بررسی Start مقصد موقتاً انجام نشد. چند لحظه بعد دوباره امتحان کن؛ هیچ Meow کم نشده است."
        return ("⛔ این کاربر از طریق بات رسمی ZIVO قابل دسترسی نیست.\n\n"
                "ازش بخواه بات رسمی ZIVO را باز کند و /start بزند؛ بعد آیدی عددی یا پیام Forward‌شده‌اش را دوباره بفرست.\n\n"
                "ℹ️ باز کردن پیوی main / acc2 / acc3 لازم نیست.")

    def handle_callback(self, raw: Dict[str, Any]) -> Optional[str]:
        self._sync_incoming_callback_identity(raw)
        return super().handle_callback(raw)
    ...
    def handle(self, raw: Dict[str, Any]) -> Optional[str]:
        ...
        uid = self._positive_numeric_id(actor.get("id"))
        chat_id = self._positive_numeric_id(chat.get("id")) or uid
        private_actor = bool(uid and chat_id == uid)
        known_before = bool(private_actor and self._official_identity_is_known(uid))
        if known_before:
            self._sync_official_identity(uid, self._actor_profile(actor))
        try:
            return super().handle(raw)
        finally:
            # New-user onboarding must inspect the pre-event persistent state
            # before we mark it seen. Enrich identity after the inherited flow
            # so Official22 keeps the established two-touch gate semantics.
            if private_actor and not known_before:
                self._sync_official_identity(uid, self._actor_profile(actor), force=True)
...
class ZivoOfficialApp:
    ...
    def run(self) -> None:
        ...
        log.info("startup | app=%s version=%s transport=official-bot-api bridge=unix-socket-direct ux=receipt-ledger-welcome-speaker-v18+cleanup-async-v19", APP_NAME, VERSION)
