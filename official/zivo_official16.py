...
"""ZIVO Official Bot v16 for Soroush Plus official Bot Platform.
...
━━━━━━━━━━━━━━━━━━
نسخه: zivo-official16 + zivo60.96.47
...
📣 اطلاع‌رسانی
کاربر می‌تواند با «تبلیغات خاموش» دریافت اطلاع‌رسانی Official را متوقف کند.
"""
    ...
    def _admin_audience(self, user_id: str, chat_id: str) -> str:
        ...
        if not result.get("ok"):
            ...
            for row in result.get("accounts") or []:
                lines.append(f"• {row.get('account_key')}: {int(row.get('private_count') or 0):,} پیوی ثبت‌شده · {row.get('status') or '-'}")
    ...
    def _admin_send_ad(self, user_id: str, chat_id: str) -> str:
        ...
        if scope in {"official","all"}:
            try:
                ...
                ids=[]
            if ids:
                threading.Thread(target=self._official_broadcast_worker,args=(ids,body),daemon=True,name="zivo-official16-broadcast").start()
